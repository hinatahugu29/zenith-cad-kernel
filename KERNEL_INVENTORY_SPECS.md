...
**Document Version:** 1.5.0 (Full Inventory & Verification Baseline)  
...
| **`zenith_algo`** | プリミティブ生成、押し出し（直進・ドラフト・中空）、回転体閉ソリッド（360度・部分角度）、配列複写（直線・円形）、**ミラー反転複写**、**3D螺旋（ヘリカル）スイープ**、ロフト・スイープ、フィレット・面取り、穴あけ、シェル化、ダイレクトモデリング、ブーリアン、スケッチ拘束ソルバー、フィーチャーツリー、物性値計算 | `zenith_math`, `zenith_geom`, `zenith_topo`, `zenith_tess` |
...
| **`zenith_py`** | PyO3 による Python C拡張（`zenith_cad.pyd`）。Blender アドオン等からのゼロコピー呼出（全34関数） | `zenith_algo`, `zenith_geom`, `zenith_topo`, `zenith_tess`, `zenith_io`, `pyo3` |
...
- 対応オペレーション: `CreateBox`, `CreateCylinder`, `CreateCone`, `CreateTorus`, `FilletEdge`, `ChamferEdge`, `HollowBox`, `ExtrudeHollow`, `ExtrudeDraft`, `RevolveSolid`, `RevolvePartialSolid`, `LoftSolid`, `SweepWire`, **`SweepHelix`**, **`MirrorSolid`**, `PushPullFace`, `ThickenFace`。
...
- **主な検証項目:**
  ...
  - `zenith_algo`: 各種プリミティブの閉ソリッド性、押し出し（直進・ドラフト・中空）、回転体ソリッド（360度・部分角度）、配列複写（直線・円形）、**ミラー反転複写**、**3D螺旋スイープ**、ロフトソリッド、任意断面ワイヤスイープ、単一エッジフィレット・面取り、穴あけ、ダイレクト Push-Pull、スケッチ拘束ソルバー、フィーチャーツリーパラメトリック自動再計算。
...
本更新を通じて、**「ミラー反転複写ビルダー」「3D螺旋（ヘリカル）スイープソリッド」「フィーチャーツリー拡張」「Python/Blender連携API拡充（全34関数）」** が実用レベルで完全実装され、全192件以上のテストスイートによって品質が担保されています。
