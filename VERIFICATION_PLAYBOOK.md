...
| `contact_placement_probe` | **B-Repゲート兼mesh既知欠陥の計測**。接触している7配置・21演算を測る（2026/08/25 時点で **19件が立体を返し、2件を断る。B-Rep非多様体0、mesh非多様体3件**。HANDOVER 4-87）。**「非多様体のはずだ」と決めつけず、B-Repの稜使用数と、派生meshの三角形辺使用数を分けて数える。** 断られること自体は赤にしない。答えが本当に非多様体なので断るものと、未実装で断るものがある。赤にするのは**非多様体B-Repを立体として返した**とき。mesh既知3件はまだexit条件ではなく、0件になった時点でゲートへ昇格する。`ZENITH_CONTACT_FILTER` と `ZENITH_CONTACT_OP` で単一配置・演算へ絞れ、`ZENITH_TESS_WHY=1` でring、earcut各段階、残るmesh edgeを表示する。 |
