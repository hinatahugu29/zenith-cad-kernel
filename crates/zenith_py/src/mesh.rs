...
impl PyMesh {
    ...
    ///
    /// **しきい値は「以上」**です（4-430）——**角がしきい値ちょうどの
    /// 稜は、残ります。**
    ///
    /// > **⚠ 模型の二面角そのものを、しきい値にしないでください。**
    /// >
    /// > **答えが割れます。** 実測（4-430。円柱 r5 h12、刻み 32）:
    /// > **縁の 256 本は、幾何としては全部 90 度**ですが、
    /// > **計算すると 89.999999999972 〜 90.000000000018 に散ります**
    /// > （90 度 ± 3e-11。**96 本が下、144 本がちょうど、16 本が上**）。
    /// >
    /// > | しきい値 | 箱 | 円柱 |
    /// > | ---: | ---: | ---: |
    /// > | 25（既定） | **12** | **256** |
    ...
    /// > | 90.01 | **0** | **0** |
    /// >
    /// > **箱は平面だけなので、ぴったり 90.000000000000**——
    /// > **割れるのは曲面のほう**です。**しきい値は、離して取って
    /// > ください**（既定の 25 は、そのためにあります）。
