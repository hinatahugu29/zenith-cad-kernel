...
> | エッジフィレット / 面取り | **「直線の稜 × 両側が平面」だけ**。他カーネルから読んだ検体14件のうち**12件は丸められる稜が 0 本**——円柱の縁、段付き軸の肩、面取り箱の稜はいずれも対象外。掛かるものは閉じた式と 1e-12〜1e-15 で一致するので、誤りではなく守備範囲（HANDOVER 4-72） |
>
> **「実用に耐えるには何が要るか」は [`HANDOVER.md`](HANDOVER.md) の9章に
> あります。** 性能（`torus × box` の積で solve 38.5秒）、フィレットの守備範囲
> （読んだ検体14件のうち12件で丸められる稜 0本）、2Dスケッチの欠落、そして
