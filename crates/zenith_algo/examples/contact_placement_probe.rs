...
fn cases() -> Vec<Case> {
    ...
    out.push(Case {
        ...
        why: "球が箱の断面に内接し、5面目（x=20）が中心を通る。**その平面は球パッチの継ぎ目と重なる**（球は経度 0/90/180/270 で分割）",
        a: boxa.clone(),
    ...
    // 中心を (10,10,20) に置くと、切る平面 z=20 は球の赤道——**継ぎ目では
    // ありません**。上の配置（x=20 で切る）は経度 0/180 の継ぎ目と重なります。
    // 接している面の数は同じなので、**差が出るならそれは接触のせいでは
    // ありません**。
    out.push(Case {
        name: "box x sphere (cut at the equator)",
        why: "上と同じ4面接触。ただし切る平面が継ぎ目を踏まない",
        a: boxa.clone(),
