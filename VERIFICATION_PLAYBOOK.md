...
| `ZENITH_CAP_TRACE=1`（環境変数） | **蓋の材料をそのまま出す**（4-218）。1つの蓋につき「稜が何本入って、ループが何本できたか」を1行で。実測: `(円柱−円柱) を箱で切る` の z=25 の蓋は **6本入って 0 ループ**——稜どうしが互いを刻んでおらず、さらに**切り手自身の稜が材料に入っていません**。材料が足りているかは、この1行で分かります |
