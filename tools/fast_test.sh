...
# 合計はおよそ 13 分（これは実測）。
#
# 並列にしたときの時間を測りました（2026年8月24日、20並列、ビルド済みから、
# 他に何も走らせていない機械）:
#
#   13分36秒（816秒）
#
# **速くなっていません。** 「いちばん遅い1本とビルドで頭打ちになるはず」と
# 書いてありましたが、実測はそうなりませんでした。理由は下の数え方にあります
# ——**examples（プローブ。2026/08/25 実測で 73 本、2026/09/08 現在 98 個）も一緒に走っています**。被覆はそのぶん増えて
# いますが、時間もそこに行っています。**816秒の内訳は測っていません。**
#
...
# そのまま流します。
#
# **数える「バイナリ」には examples（プローブ）が入ります**（2026/08/25
# に実測。現在は169本のうち73本）。`cargo test` は examples もビルドするので、下の
# `--message-format=json` が一緒に拾って走らせます。テストは 0 件ですが、
# **非ゼロ終了すれば落ちます**——プローブのゲートがそのまま掛かるぶん、
# 被覆は増えていて、そのぶん時間もかかります。切り離したくなったら
# `"executable"` を拾うところで `"target":{"kind":["example"]}` を外して
# ください。
#
