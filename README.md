...
（`screw.step`、`linkrods.step`）。同じ日に、**表示メッシュの穴を 23 分の1と
39 分の1まで減らしました**（326 → **14 本**、1330 → **34 本**。4-273〜4-285）。

**ただし目標の「恒等式の破れ 0」には届いていません。** 残っているのは
**その 14／34 本**です。**0 でないあいだは、内外判定を使う検査が読めない
数字になります**——光線の交差回数で内外を決めるので、メッシュに穴があると
数が合いません（4-276。実測で、立体の体積が面積分の値から 70% ずれました）。
