...
/// 切り込みの端を吸着してよい量の**天井**。
///
/// 大きさに比例させた許容（`tol.linear * 10 * 境界の大きさ`）は、mm 級の
/// 部品でも 3e-4 に届きます。ところが**吸着した端は曲線を曲面から浮かせ**、
/// `Face::pcurves` が「乗っている」と認めるのは **絶対 1e-5** までです。
/// 比例のほうだけを大きくすると、**割る側が通した割り方の p-curve が
/// 取れなくなり**、面積を検算できずに落ちます（4-214、4-215）。
///
/// 浮きは吸着量の **1/4** です。有理2次の弧では端の制御点の基底が真ん中で
/// `B0(1/2) = 1/4` になるためで、**曲率には依りません**——半径 1 / 2 / 5 /
/// 10 / 40 の円柱で、動かす量 1e-6 と 1e-4 の 10 通りすべてが **0.2500
/// ちょうど**でした（4-215）。だから `1e-5 / 0.25 = 4e-5` が天井です。
///
/// **いま通っているものは1つも落ちません。** `contact_placement_probe`
/// （47配置・141演算）の吸着 **1,610 回すべてが 2.714e-7 以下**で、天井の
/// 147 分の 1 です（`ZENITH_SNAP_WHY=1` で数えられます）。
///
/// **ここが絶対値なのは、`on_surface_limit` が絶対値だからです。** あちらを
