...
| 段 | 中身 | 目標 |
| :--- | :--- | :--- |
| **第0段** | **測る仕掛けを揃える**——`zenith_py` を門へ戻す（いまは `--exclude zenith_py` で回しており、130本/664件に含まれていません）。wasm を1回建てる | H1・H2 |
| **第1段** | **スケッチの幾何の背骨**——円弧 → 閉領域の抽出 → ワークプレーンの写像 → 押し出し。**拘束の編集の仕方・選択・履歴は作りません**（画面と一緒に決めるもの。HANDOVER 9-G の留保） | H3 |
| **第2段** | **フィレット／面取りの守備範囲**——読んだ15検体のうち**7件がまだ対象稜 0** | H4 |
