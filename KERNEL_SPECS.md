...
| **貫通穴あけ・ザグリ穴・六角ナット (Hole)** | `hole::HoleBuilder` | 4象限パッチ方式で上下面を四角形パッチに分割する専用貫通穴ビルダー（16面）に加え、`make_counterbore_hole_box` によるボルト頭沈めザグリ穴、および `make_hex_nut` による正六角ナットの直接生成に対応。 |
| **六角ボルト (Hex Bolt)** | `bolt::BoltBuilder` | 二面幅 $S$、頭部厚み $k$、軸部半径 $r$、軸長 $L$ から六角頭＋軸部円柱を一体化した標準六角ボルトソリッド（完全閉多様体）を生成。 |
| **段付きシャフト・キー溝 (Shaft & Keyway)** | `shaft::ShaftBuilder` | 複数段 $\&[(r_i, L_i)]$ の段付き軸（`make_stepped_shaft`）および平行キー溝の差分切削（`make_shaft_with_keyway`）による機械伝動軸の直接構築。 |
