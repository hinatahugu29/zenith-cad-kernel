...
| `cargo run --release -p zenith_algo --example face_budget_probe` | **数字**（9-H の H5）。64分割での**1面あたりの最大枚数**を、和・差・積の3演算とも出す（`ZENITH_DIVISIONS` で分割を変えられる）。**1面だけを立体にして測ってはいけません**——刻みの計画は立体の大きさと隣の稜から決まるので、切り出すと本番と違う数が出ます（実測 14,996 が 12,244 に見えました）。実測（2026/08/31）: 最悪 **14,996 枚**（25度傾けたトーラス × 箱の差）。H5 は 10,000 以下 |
