...
/// **間を埋めているのが修理**です。
///
/// **4-480 では 2 つに減りました**が、**4-484 で 4 つに戻しました**
/// ——細かい種は「粗いので 1 本も辿れなかったとき」だけに限ったためです
/// （混ぜると、他の枝を押しのけます）。
///
...
fn without_the_repair_the_intersection_curves_do_not_close() {
    ...
    // **4 つに戻りました**（4-484）。
    //
    // 4-480 で `find_seeds` が**粗い当たりの周りをもう一度見る**ように
    // したとき、ここは **4 つ → 2 つ**になりました。**4-484 で、細かい
    // 種は「粗いので 1 本も辿れなかったとき」しか使わない**ようにした
    // ので、**この検体では細かい種が回りません**（粗いので辿れます）。
    //
    // **戻したのは、混ぜると押しのけるから**です——実測（4-484）:
    // 細かい種が見つけた枝が**他の枝を締め出し**、**トーラスを 2 離した
    // 配置が返らなくなりました**。**浮いた端 2 つと、返る配置 2 つ**を
    // 秤に掛けて、**返る配置を採りました**（浮いた端は修理が埋めます）。
    assert_eq!(
        loose, 4,
