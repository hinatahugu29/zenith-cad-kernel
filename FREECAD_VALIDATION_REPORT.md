...
ブーリアン演算の対応範囲は依然として限定的です（軸平行ボックス同士は厳密、曲面が絡む多くのケースは未対応）。
ただし範囲外は誤答ではなくエラーを返すよう検証ゲートが入っており、
実際の対応範囲は `cargo run -p zenith_algo --example boolean_envelope` でいつでも測定できます。
