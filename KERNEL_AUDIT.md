...
7. Teach planar face splitting about curved boundaries. `split_planar_face_by_edge()` locates split endpoints on a coarsely sampled boundary polygon and rebuilds loops from point paths, so a chord landing on a circular cap boundary is not recognized as a boundary hit. This is the remaining blocker for a lengthwise cylinder cut: the cut face and the side rulings are now exact, but the cap faces are never split, so the result still reports unmatched edge uses instead of closing into a half-cylinder solid.
