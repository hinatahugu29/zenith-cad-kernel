...
> ## ⚠️ この文書は**設計図**です。実装の現状ではありません（2026年8月30日追記）
>
> 下の表の **✅ 完全対応** は、**この統合で満たすべき要件**の印です。**測った
> 結果ではありません。** このリポジトリの決まりは「主張ではなく測定で判断
> する」なので、線を引いておきます。**測った状態は
> [`HANDOVER.md`](HANDOVER.md) の第1章にあります。**
>
> | この文書の表現 | 実際に測ってあること |
> | :--- | :--- |
> | FreeCAD / OpenCASCADE で 100% 検証済み | **常設ゲートの範囲です。** ショーケース 54 検体、相互検証 27 件、書き戻し 7 件、IGES 5 件、非STEP出力 8 件（2026/08/27 実測）。**「どんな形でも」ではありません** |
> | ✅ 完全対応（プリミティブ各種） | **形が出て、閉じた立体として読み戻せるところまでは測ってあります。** ブーリアンでそれらを組み合わせたときは別で、`contact_placement_probe` の 35配置・105演算では **99 件が立体を返し、6 件を断ります**（2026/08/30 実測。断るのはすべて「答えが本当に非多様体だから」） |
> | Zenith CAD Kernel v1.9.0 | **版番号は文書上のもので、実測の対象ではありません。** 実測は HANDOVER の章番号で追ってください |
> | `zenith_server` 経由の連携 | **`create_stack` と `delete_stack` だけが実装されています。** 他は名前を挙げて断ります（[`SEAMLESS_PROTOCOL.md`](SEAMLESS_PROTOCOL.md)、HANDOVER 4-41）。いま先に動いているのは `zenith_py` のインプロセス経路です |
>
