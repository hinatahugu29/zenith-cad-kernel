...
> | どんな立体でもメッシュは閉じる | **常設検体の範囲です。** `mesh_watertight_probe` の検体は閉じますが、**ブーリアンが曲面を割った結果はそこに入っていません**。それを測る `contact_placement_probe`（**20配置・60演算**）では、2026/08/27 時点で **1演算にまだ非多様体の稜が 13 本**残っています（`sphere × cylinder (eccentric)` の差。着手前は 11演算・最大165本。HANDOVER 4-116〜4-128）。**その1演算は、同じ日まで断られていてメッシュ化まで来ていなかった場所です。****0 でないメッシュは STL に書けません。** B-Rep のほうは全件 0 です |
