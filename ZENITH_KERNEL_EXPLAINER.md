...
> | どんな立体でもメッシュは閉じる | **常設検体の範囲です。** `mesh_watertight_probe` の検体は閉じますが、**ブーリアンが曲面を割った結果はそこに入っていません**。それを測る `contact_placement_probe`（**30配置・90演算**）は、2026/08/30 時点で **B-Rep・メッシュとも非多様体 0** です（着手前は 11演算・最大165本。HANDOVER 4-116〜4-131）。**ただし 90演算のうち 5 件は断られていて、メッシュ化まで来ていません**——断った先に何があるかは測れていません |
