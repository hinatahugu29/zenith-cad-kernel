...
**文書バージョン**: v3.4.0 (4-97の円筒ボス根元・段付き軸肩局所フィレットを反映)
**最終更新日時**: 2026年8月26日
...
| **稜フィレット / 面取り** | `edge_blend` | 直線稜×平面2面、純直円柱の凸円周（フィレット/面取り）、純円錐/円錐台の凸円周、貫通円筒穴口（両操作）に加え、**円筒ボス根元・段付き軸の小径側90度凹円周**を局所フィレットする。自作4分割円弧、OCC全周1本円、複数段、剛体配置、STEPに対応し、未選択面と別の肩を保持。厳密有理トーラス/円錐台、閉形式最悪 5.26e-10、mesh 4〜32分割異常0。読んだ15検体で対象稜0は **12→7件**。根元面取り、非円形・非直角根元、円錐円周面取りは明示拒否（HANDOVER 4-72、4-92〜4-97）。 |
...
すべて 2026年8月25日までの実測値です。数字の出どころは
[`VERIFICATION_PLAYBOOK.md`](VERIFICATION_PLAYBOOK.md) の手順で再現できます。

| 何を測ったか | 結果 | 再現コマンド |
| :--- | :--- | :--- |
| ワークスペース全テスト | **102 バイナリ（doctest 込み）/ 581 テスト 100% 合格**（0 failed, 0 ignored） | `cargo test --release --workspace --exclude zenith_py` |
...
| FreeCAD ヘッドレス相互検証 | **22/22 完全一致**（ゲート、不一致で非ゼロ終了） | `tools/freecad_cross_validate.py` |
| OpenCASCADE ショーケース | **25/25 が valid closed solid** | `tools/verify_showcase.py` |
