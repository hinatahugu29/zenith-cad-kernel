...
/// **上流の値そのものを押さえます。**
///
/// 表示側で頂点へ寄せるのは対症です。稜の曲線の端が頂点からどれだけ
/// ずれているかを測っておくと、上流（ブーリアンの交線）が悪くなったときに
/// ここが先に鳴ります。実測（2026/08/31）: 最大 **1.330e-7**。
#[test]
fn the_gap_between_an_edge_curve_end_and_its_vertex_stays_where_it_was_measured() {
    ...
    // 悪くなったら鳴らします。**良くなったぶんは通します**——上流が直れば
    // この差は 0 に近づき、表示側の寄せは要らなくなります。
    assert!(
        worst <= 2.0e-7,
        "稜の曲線の端と頂点の差が {worst:.3e} まで開いた（2026/08/31 の実測は 1.330e-7）"
    );
}
