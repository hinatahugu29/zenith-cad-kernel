...
1. **トポロジー縫合における割り方の整合（Stitching Alignment）**:
   複雑な多面アセンブリ（例: `linkrods.step`）において、隣接面同士のトリム境界の分割数や端点接続が不揃いになる問題（相手のいない稜の発生）の解消。**現状の未達量は 3演算で 47／47／40 本**（非多様体 0 本。2026/09/06 実測）。**原因は計測済みである**——交差曲線とトリム境界は **1.1×10⁻⁴** ですれ違い、これは**ファイル自身が申告する不確かさ 2.4×10⁻⁴ を下回る**ため、**交差の有無はファイルの精度では決定できない**。なお、共有稜 94 本はすべて、当該面が申告する不確かさ（曲面公差＋p-curve 公差）の範囲内で両支持曲面上に存在する（`screw.step` も同様に 37/37）。すなわち入力データは自己整合的である。未達の所在は公差の設定ではない——縫合の照合幅を当該面の申告不確かさに置き換えても未整合稜数は 47／47／40 のまま不変であり（追試済み）、未整合稜から最近傍の対応稜までの距離分布は 0 または 0.01 以上に二分され、$10^{-6}$–$10^{-2}$ の区間が完全に空である。したがって本問題は「両オペランドが異なる位置で面分割されている」という位相的不整合であり、許容幅の調整では解けない（許容幅を広げる試行は 6 回とも改善しなかった）。
2. **表示メッシュの水密性の測定範囲**:
