...
> 数えて印字するだけです。**2026/08/28 の実測は 30配置・90演算で 81 成功・9 拒否、
> B-Rep 非多様体 0、mesh 非多様体は 1演算（13本）**です（同 4-116〜4-142）。
> これは常設検体の実測範囲で、任意の全立体の証明ではありません。
>
> **ブーリアンには、まだ誤答が1件あります。** 他カーネルが書いた `cylinder` と
> `elliptic_prism` を重ねると、和と差が真値より 150.951 大きくなります
> （積は正しい。相対 1.2%。HANDOVER 4-142、未解決）。
