...
> 「169 バイナリ」には**プローブ（examples、73本）が入る**ので、被覆が
...
**最終確認**: 2026年8月26日（HANDOVER 4-97 まで）
...
**期待**: 102 テストバイナリ（doctest 込み）、581 テスト、失敗 0。
...
| `foreign_edit_probe` | **ゲート**。他カーネルから読んだ立体15件に、フィレット・面取り・プッシュプル・面の計測を掛ける。**掛からないことは欠陥として数えない**（実装していない組み合わせがある）。数えるのは「掛かったのに答えが違う」ほう——壊れた立体、体積の符号が逆、閉じた式から外れる。純円柱の両操作、純円錐/円錐台、穴口の両操作、段付き軸根元の材料追加を閉じた式で採点し、2026/08/26時点で **17操作適用・閉形式19件・WRONG 0**（4-92〜4-97）。 |
...
| `target/validation/` | 相互検証用22形状＋OCC が書いた参照ファイル | `freecad_cross_validate.py`（22/22のゲート） |
