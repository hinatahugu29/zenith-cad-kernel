...
で **150 バイナリ / 739 件通過 / 失敗 0 / 無視 0 / 警告 0**。
...
- **常設プローブ（診断・ゲート）:** `crates/zenith_algo/examples` に **117 本**（2026/09/19 実測。`ls crates/zenith_algo/examples/*.rs | wc -l` で数えられます）。一覧と、それぞれ何を赤にするかは [`VERIFICATION_PLAYBOOK.md`](VERIFICATION_PLAYBOOK.md) の道具表に。CI は `.github/workflows/gates.yml`
