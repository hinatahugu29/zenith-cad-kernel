...
| `contact_placement_probe` | **ゲート**。接触している配置を演算ごとに測る。**「非多様体のはずだ」と決めつけず、返ってきた立体のメッシュで稜がちょうど2枚に共有されているかを数える。** 断られること自体は赤にしない（未実装のことがある）。赤にするのは**非多様体を立体として返した**とき。同じ4面接触を、切る平面が球パッチの継ぎ目を踏む向きと踏まない向きの両方で持っているので、**接触のせいか継ぎ目のせいかが1枚で分かる**（HANDOVER 3-1） |
