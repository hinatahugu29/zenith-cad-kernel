...
> 「165 バイナリ」には**プローブ（examples、73本）が入る**ので、被覆が
...
**最終確認**: 2026年8月25日（HANDOVER 4-91 まで）
...
**期待**: 98 テストバイナリ（doctest 込み）、532 テスト、失敗 0。
...
| `foreign_edit_probe` | **ゲート**。他カーネルから読んだ立体14件に、フィレット・面取り・プッシュプル・面の計測を掛ける。**掛からないことは欠陥として数えない**（実装していない組み合わせがある）。数えるのは「掛かったのに答えが違う」ほう——壊れた立体、体積が増える、閉じた式から外れる。丸められる稜が何本あるかも出るので、守備範囲が狭まったら気づける（4-72） |
