...
**残っているのは表示メッシュの穴**です。9月2日に **326 → 14 本**、
**1330 → 34 本**（23 分の1と 39 分の1）まで減らしました（4-277〜4-285）。
**0 でないあいだは、内外判定を使う検査が読めない数字になります**
（4-276）。**次の一手はここです。**
...
| STEP インポーター | **着手済み**（2026/09/02。同じ日に**表示メッシュの穴を 326 → 14 本、1330 → 34 本**まで減らしました。4-285）。OCCT が配る実物の `screw.step`・`linkrods.step` が**読めて切れます**。円錐の頂点の向こう側（4-261）、紡錘トーラスの内枝（4-264）を直し、**面が自分の粗さを持ち歩く**ようにしました（4-266）。残るのは速度と非多様体（4-269〜4-271） |
