...
| `ZENITH_STEP_WHY=1`（環境変数） | **診断**。STEP を読むとき、**稜ごとに「曲線の端が頂点からどれだけ離れているか」**を、曲線の実体の種類と `same_sense` つきで出します（公差を超えたものだけ）。読めなかったファイルで、悪いのが曲線なのか面なのかを分けるための口です。実測（4-228）: `screw.step` で 2 本が **1.018357e1** ずれており、**稜が B スプラインを途中までしか使わないのに切り詰めていない**と分かりました。直したあとは 0 本です |
