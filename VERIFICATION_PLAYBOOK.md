...
| `ZENITH_TESS_WHY=1`（環境変数） | **診断**。面ごとに、**稜を共有する経路に乗ったか**（乗らなければ理由）と、リングの点数・稜ごとの刻みを出す。両側が同じ稜を違う数で刻んでいれば、そこでメッシュが開く。実測ではこれで「刻みは合っているのに大円の上の頂点が12倍ある」ところまで絞れた（HANDOVER 4-83、3-N-2b） |
