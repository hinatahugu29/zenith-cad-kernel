...
> 「162 バイナリ」には**プローブ（examples、72本）が入る**ので、被覆が
...
**最終確認**: 2026年8月25日（HANDOVER 4-80 まで）
...
**期待**: 94 テストバイナリ（doctest 込み）、512 テスト、失敗 0。
...
| `contact_placement_probe` | **B-Repゲート兼mesh既知欠陥の計測**。接触している7配置・21演算を測る（2026/08/25 時点で **19件が立体を返し、2件を断る。B-Rep非多様体0、mesh非多様体1件・3本**。HANDOVER 4-88）。**「非多様体のはずだ」と決めつけず、B-Repの稜使用数と、派生meshの三角形辺使用数を分けて数える。** 断られること自体は赤にしない。答えが本当に非多様体なので断るものと、未実装で断るものがある。赤にするのは**非多様体B-Repを立体として返した**とき。mesh既知1件はまだexit条件ではなく、0件になった時点でゲートへ昇格する。`ZENITH_CONTACT_FILTER` と `ZENITH_CONTACT_OP` で単一配置・演算へ絞れ、`ZENITH_TESS_WHY=1` でring、earcut各段階、残るmesh edgeを表示する。 |
...
`.github/workflows/gates.yml` の一覧が権威で、いまは 27 本です。
`crates/zenith_algo/examples/` には 65 本の実行ファイルがあり、**CI に
