...
| `ZENITH_EARCUT_WHY=1`（環境変数） | **診断**。面ごとに、**リングの境界の辺のうち三角形分割に現れなかった本数**を出す。現れない辺は隣の面から見て**相手のいない稜**になり、メッシュに穴が開きます。earcut は共線・重複の点を自分で落とすので、挿し戻し（4-86）と耳の修理が入っていますが、**通り抜けるものが残ります**。実測: `cone × torus` を持ち上げた配置で、**145本のうち1本**が2枚の面で欠けていました（4-207） |
