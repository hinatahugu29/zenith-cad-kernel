...
### 3-2b. 読み込んだ形状を書き出し直すと、自分で読めなくなるものがある

インポーターが作った形状を STEP に書き戻す経路は、今回初めて通しました。
...
OpenCASCADE は有理B-spline を解析曲面と同じようには測りません。自分の円柱を
`toNurbs` してから測ると 12674.63（解析値 12566.37 に対して +0.86%）です。
なので比較相手は「OCC 自身の NURBS 化」であって「OCC の解析値」ではありません。

| 対象 | OCC 解析 | OCC 自身の NURBS | OCC が見た我々の再出力 |
| :--- | ---: | ---: | ---: |
| 円柱 | 12566.3706 | 12674.6348 | 12665.8892 |
| 円錐 | 3267.2564 | 3293.4785 | 3291.6037 |
| 頂点まで届く円錐 | 2094.3951 | 2110.2985 | **2093.6988** |
| トーラス | 3789.9281 | 3799.1130 | 3797.2856 |
| トーラス区分 | 947.4820 | 948.1253 | 948.1995 |
| 半球 | 2094.3951 | 2117.0016 | **2093.9989** |
| 球 | 4188.7902 | 4186.9583 | **4227.6111** |

7件中6件は、OCC 自身の NURBS 化と同等かそれより良い値になっています
（頂点まで届く円錐と半球は明確に良い）。**なお自前リーダーは、これら再出力を
すべて解析値どおり 1e-13 で読み直します**（`foreign_reexport` の右側の列）。

**残っている2件**

1. **球だけが外れ値**（+0.93%、OCC 自身の NURBS 化は -0.04%）。
   インポーターは全周を1枚のパッチで作ります。自前では厳密ですが、
   OpenCASCADE は全周を巻き付いた1枚の面を苦手とします。
   これは既知の落とし穴で、**自前のビルダーは同じ理由で球を8枚に分割しています**
   （`KERNEL_SPECS.md` の球の行に経緯あり）。インポーターは分割していません。
   直すなら、書き出し時に巻き付き面を分ける（面のトリムループも割る必要あり）か、
   読み込み時に分けて作る。

2. **半球（`sphere_capped`）の再出力を、自前リーダーが読めません。**
   「境界点が曲面から 1.827273 外れている」で弾かれます。
   書いたファイル自体は構造的に正しく（9×3 格子、重みもノットも整合）、
   元ファイルの読み込みでは境界は曲面上 5e-13 に乗っています。
   **1.827273 は、3-2 の `pcurve_fidelity_probe` がこの面に対して出す値と同じ数**です。
   同じ欠陥の別の顔である可能性が高いので、3-2 を直してから再測してください。

**測り方の注意（またやりました）**: 境界が曲面に乗っているかを 80×80 の格子で
測ったところ 0.41 という値が出ましたが、これは**格子の間隔そのもの**でした
（半径10の全周を80分割 → 0.39）。粗い格子で当たりを付けてから局所的に詰めると
5e-13 です。存在しない欠陥を追いかけるところでした。
