...
fn test_feature_tree_extended_features() {
    ...
    let expected = 40.0 * 30.0 * 20.0
        + t * 20.0 * 20.0 * (40.0 + 30.0)
        + (4.0 / 3.0) * t * t * 20.0f64.powi(3);
    ...
    let solid_cs = tree_cs
        .recompute()
        .expect("evaluate countersunk screw tree");
    ...
    let solid_bs = tree_bs
        .recompute()
        .expect("evaluate belleville spring tree");
    ...
    let solid_recomputed = deserialized_tree
        .recompute()
        .expect("recompute from json tree");
    assert!(solid_recomputed
        .outer_shell
        .validate_closed(&tol)
        .is_valid());
}
