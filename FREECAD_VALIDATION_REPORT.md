...
>
> **まだ誤答が1件残っています**（`cylinder` × `elliptic_prism`。4-142）。
...
**81 成功・9 拒否・非多様体を返したもの 0** です（2026/08/28 実測）。
...
| **他カーネルの立体どうし**（10検体・45組） | 45 | **誤答 1**（下記） |

**まだ誤答が1件残っています。** OpenCASCADE が書いた `cylinder` と
`elliptic_prism` を中心を揃えて重ねると、**和と差が真値より 150.951 大きく**
なります（積は正しい。相対 1.2%）。分割数を 24 / 48 / 96 と変えても値は
1ビットも動かないので、テッセレーションではなく幾何の誤りです。
**検証ゲートも内外一貫性も通り、恒等式でしか見えません**（HANDOVER 4-142、
未解決）。
