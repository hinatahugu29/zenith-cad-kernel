...
> | 項目 | 実測 |
> | :--- | :--- |
> | ブーリアン | 自作立体・他カーネルの立体とも、軸に平行な切り手でも27度傾けた切り手でも**断るものは無し**（180演算、WRONG 0・PANIC 0）。**検証つきの口でも 180/180**（HANDOVER 4-67、4-68）。自作立体どうしの45ケースは **supported 44 / wrong-result 0 / エラー 1**（8月24日は 39 / 0 / 6。HANDOVER 4-74・4-78〜4-80）。**残る1件は直す対象ではありません**——`box × cylinder`（接線）の差で、**答えのほうが非多様体**なので場所を名指しして断ります（規約: 接触は位相を作らない）。ここが `ok` に変わったら誤答です。**45ケースに無い置き方も測っています**（`contact_placement_probe`、15配置・45演算）。2026/08/27 に4配置を足し、**箱と円柱を両方回した配置**が3演算とも断られていたのを解消しました（原因は面積の検算が安い物差しだけで正しい割りを捨てていたこと。HANDOVER 4-119〜4-122）。立体を返す演算は 34 → **37**、**B-Rep の非多様体は全件 0**。派生メッシュの非多様体は 11演算 → **1演算**（4-116、4-118、4-123、4-125） |
