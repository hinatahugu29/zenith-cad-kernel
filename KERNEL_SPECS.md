...
**文書バージョン**: v3.3.5 (4-92の純円柱円周フィレットを反映)
**最終更新日時**: 2026年8月25日  
...
| **稜フィレット / 面取り** | `edge_blend` | 直線稜×平面2面の有理2次円筒フィレット/平面面取りに加え、**純直円柱の凸キャップ円周**を厳密有理トーラス4枚で丸める。自作4分割円弧、OCC全周1本円、剛体配置後に対応し、1円弧選択を全周へ伝播。閉形式最悪 9.52e-11、STEP外部照合、mesh 4〜32分割異常0。読んだ14検体で丸められる稜0は **12→11件**。穴・ボス・段付き軸等の複合立体と円形面取りはまだ明示拒否（HANDOVER 4-72、4-92）。 |
...
| 何を測ったか | 結果 | 再現コマンド |
| :--- | :--- | :--- |
| ワークスペース全テスト | **99 バイナリ（doctest 込み）/ 543 テスト 100% 合格**（0 failed, 0 ignored） | `cargo test --release --workspace --exclude zenith_py` |
...
| FreeCAD ヘッドレス相互検証 | **16/16 完全一致**（ゲート、不一致で非ゼロ終了） | `tools/freecad_cross_validate.py` |
