...
> 「167 バイナリ」には**プローブ（examples、73本）が入る**ので、被覆が
...
**最終確認**: 2026年8月25日（HANDOVER 4-94 まで）
...
**期待**: 100 テストバイナリ（doctest 込み）、561 テスト、失敗 0。
...
| `foreign_edit_probe` | **ゲート**。他カーネルから読んだ立体14件に、フィレット・面取り・プッシュプル・面の計測を掛ける。**掛からないことは欠陥として数えない**（実装していない組み合わせがある）。数えるのは「掛かったのに答えが違う」ほう——壊れた立体、体積が増える、閉じた式から外れる。純円柱の両操作と純円錐/円錐台フィレットを閉じた式で採点し、2026/08/25時点で **12操作適用・閉形式14件・WRONG 0**（4-92〜4-94）。 |
...
| `target/validation/` | 相互検証用19形状＋OCC が書いた参照ファイル | `freecad_cross_validate.py`（ゲート） |
