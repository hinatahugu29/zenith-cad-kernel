...
> 数えて印字するだけです。**2026/09/04 の実測は 47配置・141演算で 135 成功・6 拒否、
> B-Rep 非多様体 0、mesh 非多様体 0**です（8月30日は 35配置・105演算で 99 / 6、
> 8月28日は 30配置・90演算で 81 / 9、mesh は 1演算・13本。
> HANDOVER 4-116〜4-131、4-169〜4-226）。拒否の 6 件は
> **すべて「答えが本当に非多様体だから」**で、場所を名指しします。
> **「未実装だから」の拒否は 0 件**で、そこは赤ゲートになりました（4-226）。
> **恒等式も測るようになりました**（**41配置**・破れ 0・残差の最悪 3.900e-8。
> 4-191）——
...
| | `solve_2d_sketch` | JSON定義スケッチの2D拘束ソルバー解計算。`(points_json, constraints_json)` の**2引数**。**知らない拘束の種類・範囲外の点番号・`value` の欠落は、名指しで断ります**（2026/09/08。HANDOVER 4-401。**それまでは黙って落としており、綴りを 1 文字間違えると何も掛からない図が「解けた」と返っていました**） |
| **Sketch → Solid** | `Solid.from_sketch_extruded` | `(sketch_json, height)` $	o$ スケッチを押し出して立体に。**穴も渡せます**（輪が 2 つ以上なら、いちばん広いものを外周とし残りを穴に）。2026/09/08 追加（4-400、4-392） |
| | `Solid.from_sketch_revolved` | `(sketch_json, axis_x, axis_y, dir_x, dir_y)` $	o$ スケッチを回して立体に。**軸はスケッチの座標**で、**輪が軸に触れる／またぐ場合は断ります**。2026/09/08 追加（4-400、4-391） |

---

## 3. テストスイート検証結果

**2026年9月22日の実測**: `cargo test --release --workspace --exclude zenith_py`
で **154 バイナリ / 745 件通過 / 失敗 0 / 無視 0 / 警告 0**。
...
- **常設プローブ（診断・ゲート）:** `crates/zenith_algo/examples` に **118 本**（2026/09/19 実測。`ls crates/zenith_algo/examples/*.rs | wc -l` で数えられます）。一覧と、それぞれ何を赤にするかは [`VERIFICATION_PLAYBOOK.md`](VERIFICATION_PLAYBOOK.md) の道具表に。CI は `.github/workflows/gates.yml`
