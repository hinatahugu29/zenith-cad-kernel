...
**2026/09/06 に、測った 10 通りすべて（6・8・10・12・16・20・24・32・48・64）で水密になった**（`screw.step`・`linkrods.step` とも開放境界 0・非多様体 0。破れの合計は 472 → 147 → 21 → 0）。**単一の分割数だけを見るのは
測り方の穴であり、24分割が水密だったのは偶然である。** 表示メッシュの
分割非依存な水密化は未達である（§7-2）。

もう一方の実物データ `linkrods.step`（37面・NURBS 31/平面 6）は、
**3演算とも 132〜144 秒で返るが、縫合が閉じないためソリッドを返さず拒否する**。
現状の未達量は**相手のいない稜 46／46／40 本、非多様体 0 本**である
（2026/09/07 実測）。**もっともらしいソリッドを返すより、拒否して数値を
...
* **メモリ安全性**: **137テストバイナリ・697件**の網羅的テストスイート（`cargo test --workspace --release`。2026/09/07 実測、失敗 0・警告 0）において、メモリリークおよびセグメンテーション違反の発生は **0件** を維持している。コードベース全体に `unsafe` ブロックは **0箇所**である（PyO3 が生成する FFI 境界を除く）。
...
1. **トポロジー縫合における割り方の整合（Stitching Alignment）**:
   複雑な多面アセンブリ（例: `linkrods.step`）において、隣接面同士のトリム境界の分割数や端点接続が不揃いになる問題（相手のいない稜の発生）の解消。**現状の未達量は 3演算で 46／46／40 本**（非多様体 0 本。2026/09/07 実測）。**原因は計測済みである**——交差曲線とトリム境界は **1.1×10⁻⁴** ですれ違い、これは**ファイル自身が申告する不確かさ 2.4×10⁻⁴ を下回る**ため、**交差の有無はファイルの精度では決定できない**。なお、共有稜 94 本はすべて、当該面が申告する不確かさ（曲面公差＋p-curve 公差）の範囲内で両支持曲面上に存在する（`screw.step` も同様に 37/37）。すなわち入力データは自己整合的である。未達の所在は公差の設定ではない——縫合の照合幅を当該面の申告不確かさに置き換えても未整合稜数は当時の 47／47／40 のまま不変であり（追試済み）、未整合稜から最近傍の対応稜までの距離分布は 0 または 0.01 以上に二分され、$10^{-6}$–$10^{-2}$ の区間が完全に空である。すなわち許容幅の調整では解けない（許容幅を広げる試行は 6 回とも改善しなかった）。**所在は面分割器の機能範囲である**——本カーネルが NURBS 面を分割できるのは「四辺形の円柱側面パッチを、母線または軸まわりの円で切る」場合に限られ、先行するブーリアンで切り欠きの入った面（外周稜が 5 本以上）は分割できない。**さらに、その円柱側面パッチの認識器は本カーネル自身のビルダーが生成する表現（次数 2×1・制御点 3×2 の四半円パッチ）のみを受理するため、読み込んだ 2 ファイルの曲面 37 枚のうち該当は 0 枚である**（`step_surface_shape_probe` による実測。`linkrods.step` の曲面は次数 6×8・制御点 7×286 に達するものを含み、4 分の 3 は $v$ 方向の次数が 1 でない自由曲面である）。**これは読み込みデータ固有の問題ではなく**、自作ソリッドの連鎖ブーリアンにおいても同一の制限に到達することを確認した。また、分割曲線が面の内部で終端する事例（`linkrods.step` で 81 件、境界からの距離の中央値 0.345）は、面を 2 領域に分割する現行の枠組みでは扱えない。解決には、**表現に依存しない uv 空間の任意ループ面分割**と、**面をまたいで連続する切り口を単一ループとして組み立てる段**の実装を要する。
2. **表示メッシュの水密性の測定範囲**:
   読み込んだ実物 STEP 2 件は**測った 11 通りの分割数すべてで水密**である（§5.2）が、これは**常設検体の実測範囲**であって分割非依存性の証明ではない。**単一の分割数での測定は健全性の証明にならない**ため、分割を振る掃き出しを門として常設している（自作立体は 4〜64 の 11 通り、読み込みファイルは 6〜64 の 10 通り）。
