...
**トーラスが次に近い**はずです。軸に垂直な平面はトーラスを2本の円で切り、
それは副角一定の曲線＝**パッチの v 等パラメータ線**です。ただし2つ足りません。

1. `cylinder_section_curve` は `degree_v == 1` 専用です。トーラスは 2 次なので、
   一般の等パラメータ線の取り出し（同次座標で v 方向の基底を評価）が要ります。
2. `edge_lies_on_recognized_cylinder_patch` は `radius_at` が軸方向に線形である
   ことを前提にしています。トーラスでは成り立たないので、専用の認識が要ります。

一方で、円弧の分類（軸座標が等しい辺が断面）と母線の対応づけ（軸まわりの角度が
等しい）は、トーラスでもそのまま成り立つはずです。90度パッチの v 境界は
副角 0 度と 90 度で、それぞれ z 一定になります。
