...
fn main() {
    ...
    //
    // 実測（2026/08/28）: 3検体・3組で 143 秒、**6検体・15組で 722 秒**
    // （14組が3演算そろい、恒等式の破れ 0、残差の最悪 9.234e-10）。CI の
    // 1本あたりの上限は 1200 秒なので、**余裕は 478 秒**です。
    //
    // **広いほうを既定にした理由。** 2026/08/28 に直した誤答は3件とも
    // **恒等式でしか見えませんでした**——面は閉じ、非多様体でもなく、内外
    // 判定も通ります（4-143〜4-145）。**この掃き出しが唯一の網**なので、
    // 目が粗いと素通りします。3組では `cylinder × elliptic_prism` しか
    // 当たりません。
    //
    // **短いほうが要るときは `ZENITH_PAIR_QUICK=1`。** 手元で1組だけ見たい
    // ときは `ZENITH_PAIR_FILTER` のほうが速いです。
    //
    // **さらに広げるときは、まず実行時間を測ってください。**
    let mut names = vec!["cylinder", "elliptic_prism", "sphere"];
    if std::env::var_os("ZENITH_PAIR_QUICK").is_none() {
