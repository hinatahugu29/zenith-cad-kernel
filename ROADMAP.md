...
> | 項目 | 実測 |
> | :--- | :--- |
> | ブーリアン | 自作立体・他カーネルの立体とも、軸に平行な切り手でも27度傾けた切り手でも**断るものは無し**（180演算、WRONG 0・PANIC 0）。**検証つきの口でも 180/180**（HANDOVER 4-67、4-68）。自作立体どうしの45ケースは **supported 44 / wrong-result 0 / エラー 1**（8月24日は 39 / 0 / 6。HANDOVER 4-74・4-78〜4-80）。**残る1件は直す対象ではありません**——`box × cylinder`（接線）の差で、**答えのほうが非多様体**なので場所を名指しして断ります（規約: 接触は位相を作らない）。ここが `ok` に変わったら誤答です。**45ケースに無い置き方も測っています**（`contact_placement_probe`）。2026/08/27 に配置を 15 → **30** へ増やしました。**2026/08/30 の実測は 35配置・105演算で 99 件が立体を返し、6 件を断ります**（8月28日は 30配置・90演算で 81 / 9）。同じ日に、**断る 6 件がすべて「答えが本当に非多様体だから」になりました**——どこでそうなるかを座標で名指しします。**「未実装として断る」は 0 件**です（HANDOVER 4-169〜4-197）。**この門は答えの正しさも見るようになりました**——恒等式（`|A∪B|+|A∩B|=|A|+|B|`、`|A＼B|+|A∩B|=|A|`）を 29配置で測り、破れ 0・残差の最悪 3.900e-8。それまでは**多様体かどうかしか見ていませんでした**（4-191）。同じ日に、**この測定自体が実行ごとに揺れていた**ことも見つけて直しました（`HashSet` の反復順が細分の答えを決めていた。HANDOVER 4-132）。さらに**総当たりの掃き出し**（中に入っている／離れている／まったく同じ／公差の前後だけずらす／半分重ね／回して重ねる／スケール／入れ子／引数の順序／結合則／面の一部だけの接触／**他カーネルの立体どうし**）で、**もっともらしい立体を返す誤答を3つ**と、**本番経路のパニック1件**を見つけて直しました（HANDOVER 4-133〜4-141）。**2026/08/28 には、残っていた誤答1件も解決しました**——他カーネルの `cylinder × elliptic_prism` で、原因は `earcut` の出力を面積で検算していなかったこと（恒等式の残差 1.2e-2 → 9.234e-10。4-142、4-143）。**同じ日に、空洞のある立体のブーリアンも開通しました**——それまで `hollow_box` はあらゆる相手との3演算が断られていました（4-144、4-145）。**B-Rep の非多様体は全件 0**、派生メッシュの非多様体も **全件 0**（着手前は 11演算・最大165本。4-116〜4-131）。同じ日に、**曲面どうしのブーリアンが通るようになりました**——直交する2円柱（等半径。Steinmetz 立体 $16R^3/3=1152$ に相対差 7.4e-8）と偏心する球×円柱の6演算（HANDOVER 4-128）。**ただし全配置の証明ではありません。測った組だけです** |
