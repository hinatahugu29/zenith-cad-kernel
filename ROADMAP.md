...
> | 項目 | 実測 |
> | :--- | :--- |
> | ブーリアン | 自作立体・他カーネルの立体とも、軸に平行な切り手でも27度傾けた切り手でも**断るものは無し**（180演算、WRONG 0・PANIC 0）。**検証つきの口でも 180/180**（HANDOVER 4-67、4-68）。自作立体どうしの45ケースは **supported 44 / wrong-result 0 / エラー 1**（8月24日は 39 / 0 / 6。HANDOVER 4-74・4-78〜4-80）。**残る1件は直す対象ではありません**——`box × cylinder`（接線）の差で、**答えのほうが非多様体**なので場所を名指しして断ります（規約: 接触は位相を作らない）。ここが `ok` に変わったら誤答です。ただし**45ケースに無い置き方には、まだ欠陥があります**（HANDOVER 3-N-2） |
...
> | DXF | 4レイヤーを定義するが、自動割り当ては OUTLINE / HOLE のみ。線種は全て CONTINUOUS、HATCH は未出力 |
...
> | エッジフィレット / 面取り | 直線稜×平面2面、純直円柱の凸円周（フィレット/面取り）、純円錐/円錐台の凸円周に加え、**貫通円筒穴口の局所フィレット/面取り**を処理。読んだ15検体で対象稜0は **12→8件**。ボス根元・段付き軸肩と円錐円周面取りはまだ対象外（HANDOVER 4-72、4-92〜4-96） |
>
> **「実用に耐えるには何が要るか」は [`HANDOVER.md`](HANDOVER.md) の9章に
> あります。** 性能（`torus × box` の積で solve 38.5秒）、フィレットの守備範囲
> （貫通穴口の両操作まで対応済みだが、ボス根元・段付き軸肩は未対応）、2Dスケッチの欠落、そして
