...
**最終確認**: 2026年8月25日（HANDOVER 4-95 まで）
...
**期待**: 101 テストバイナリ（doctest 込み）、572 テスト、失敗 0。
...
| `foreign_edit_probe` | **ゲート**。他カーネルから読んだ立体15件に、フィレット・面取り・プッシュプル・面の計測を掛ける。**掛からないことは欠陥として数えない**（実装していない組み合わせがある）。数えるのは「掛かったのに答えが違う」ほう——壊れた立体、体積が増える、閉じた式から外れる。純円柱の両操作、純円錐/円錐台、複数穴プレートの穴口フィレットを閉じた式で採点し、2026/08/25時点で **14操作適用・閉形式15件・WRONG 0**（4-92〜4-95）。 |
...
| `target/validation/` | 相互検証用20形状＋OCC が書いた参照ファイル | `freecad_cross_validate.py`（ゲート） |
