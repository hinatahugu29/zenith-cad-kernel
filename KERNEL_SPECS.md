...
| **貫通穴あけ・ザグリ穴・皿モミ穴・六角ナット (Hole)** | `hole::HoleBuilder` | 4象限パッチ方式で上下面を四角形パッチに分割する専用貫通穴ビルダー（16面）に加え、`make_counterbore_hole_box` によるボルト頭沈めザグリ穴、`make_countersink_hole_box` による皿ビス座面穴、および `make_hex_nut` による正六角ナットの直接生成に対応。 |
