...
> | どんな立体でもメッシュは閉じる | **常設検体の範囲です。** `mesh_watertight_probe` の検体は閉じますが、**ブーリアンが曲面を割った結果はそこに入っていません**。それを測る `contact_placement_probe`（15配置・45演算）では、2026/08/27 時点で **1演算にまだ非多様体の稜が 30 本**残っています（着手前は 11演算・最大165本。HANDOVER 4-116〜4-125）。**0 でないメッシュは STL に書けません。** B-Rep のほうは全件 0 です |
