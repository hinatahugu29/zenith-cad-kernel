...
> | | |
> | :--- | :--- |
> | **何であるか** | **個人ひとりと AI が 18 日で書いた B-Rep CAD カーネル**（854 コミット、Rust 119,293 行、8 crate、掃き出し 90 本、テスト 697 件） |
> | **できること** | 厳密なブーリアン（141 演算で恒等式の破れ 0）、STEP の読み書き、水密なメッシュ、実物の `screw.step` を読んで切る |
> | **できないこと** | 実物の `linkrods.step` はまだ切れない（縫合で相手のいない稜が **46／46／40 本**。**非多様体は 0**。2026/09/07 実測）。**根まで測りました**——**公差の話ではなく**（相手までの距離は 0 か 0.01 以上で、間が丸ごと空。4-291）、**ファイルが壊れているのでもありません**（共有する稜 94 本は、面が申告する粗さの中ですべて両方の曲面に乗ります。4-351）。**残っているのは、書いていない機能**です——**NURBS の面を割る道は「四辺形の円柱側面を、母線か円で切る」しかなく**、切り欠きの入った面は割れません（4-367）。**しかもその認識器は、自分のビルダーが作った持ち方しか受け付けません**——**読んだ 2 ファイルの曲面 37 枚のうち、当てはまるのは 0 枚**です（4-371）。**これは読んだファイル特有の問題ではありません**——**自作の立体の連鎖ブーリアンでも、同じ壁に当たります**（4-366） |
...
> | スライサーでエラーが一切起きない | **測っていません。** 測ってあるのは「**自分で作った立体は** 4〜64 分割の 11 通りすべてで open 0・非多様体 0・退化 0」（`mesh_watertight_probe`。2026/09/06 に**門にしました**——それまでは**開いていても終了コード 0** でした。4-348。`mesh_density_probe` でも 11 通りの刻みで確かめてあります。4-297、4-348）です。**読み込んだファイルも、いまは 10 通りすべてで水密です**——`screw.step` は 16・20・32・48 分割で壊れていましたが、2026/09/06 に **0** になりました（4-296、4-329、4-332、4-338、4-347）。スライサーに掛けた記録はありません |
