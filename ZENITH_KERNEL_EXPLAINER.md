...
> | どんな立体でもメッシュは閉じる | **常設検体の範囲です。** `mesh_watertight_probe` の検体は閉じますが、**ブーリアンが曲面を割った結果はそこに入っていません**。それを測る `contact_placement_probe`（**30配置・90演算**）では、2026/08/28 時点で **1演算にまだ非多様体の稜が 13 本**残っています（`sphere × cylinder (eccentric)` の差。着手前は 11演算・最大165本。HANDOVER 4-116〜4-142）。**その1演算は、断られていてメッシュ化まで来ていなかった場所です。****0 でないメッシュは STL に書けません。** B-Rep のほうは全件 0 です |
> | ブーリアンはいつでも正しい答えを返す | **違います。** 2026/08/28 時点で、**誤答が1件残っています**——他カーネルが書いた `cylinder` と `elliptic_prism` を重ねると、和と差が真値より 150.951 大きくなります（積は正しい。相対 1.2%。HANDOVER 4-142）。検証ゲートは通ってしまい、**恒等式でしか見えません**。同じ日に、同種の誤答を3件と本番経路のパニックを1件直しました（4-133〜4-141）が、**「全部直った」とは言えません** |
