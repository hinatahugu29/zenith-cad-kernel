...
**文書バージョン**: v3.3.9 (4-96の貫通穴口局所面取りを反映)
**最終更新日時**: 2026年8月26日
...
| **稜フィレット / 面取り** | `edge_blend` | 直線稜×平面2面、純直円柱の凸円周（フィレット/面取り）、純円錐/円錐台の凸円周に加え、**平面板を貫通する円筒穴の口**を局所再トリムしてフィレット/面取りする。自作4分割円弧、OCC全周1本円、全B-spline STEPの平面化後、上下口、複数穴、剛体配置に対応し、1円弧選択を全周へ伝播。未選択面を保持し、厳密有理トーラス/円錐台、閉形式最悪 1.39e-10、STEP外部照合、mesh 4〜32分割異常0。読んだ15検体で対象稜0は **12→8件**。ボス根元・段付き軸肩、円錐円周の面取りは明示拒否（HANDOVER 4-72、4-92〜4-96）。 |
...
| 何を測ったか | 結果 | 再現コマンド |
| :--- | :--- | :--- |
| ワークスペース全テスト | **101 バイナリ（doctest 込み）/ 575 テスト 100% 合格**（0 failed, 0 ignored） | `cargo test --release --workspace --exclude zenith_py` |
...
| FreeCAD ヘッドレス相互検証 | **21/21 完全一致**（ゲート、不一致で非ゼロ終了） | `tools/freecad_cross_validate.py` |
