...
fn cases() -> Vec<Case> {
    ...
    out.push(Case {
        ...
        note: [
            "実測: 縫えずに断られる（あぶれた稜 12、非多様体 0、同方向 0。2026/08/30。8月28日は 48 と 24）",
            "実測: 場所を名指しして断る（(19.29,17.59,20) で材料が2つに割れる）。**規約どおり**",
            "実測: 立体を返す（4-182 で通るようになった。8月28日はあぶれた稜 56 で断られていた）",
        ],
    });
    ...
    out.push(Case {
        ...
        note: [
            "実測: **場所を名指しして断る**（(6,0,-10) で材料が2つに割れる）。**規約どおり**——答えが本当に非多様体（母線1本を4枚の面が共有する）。2026/08/30 まで「未実装」と言っていた（4-131、4-170、4-190）",
            "実測: A がそのまま返る（立体1つ）",
            "実測: 立体0つ。線は体積を持たない",
        ],
    });
    ...
    out.push(Case {
        ...
        note: [
            "実測: A がそのまま返る（B は中に入っている）。**4-197 で通るようになりました**——8月30日の途中までは非多様体の稜 16本で断られていた",
            "実測: 場所を名指しして断る——接する母線で壁の厚みが 0 になる。**規約どおり**",
            "実測: B がそのまま返る",
        ],
    });
    ...
    // 面で触る（平面どうし）。箱を横に並べ、側面をぴったり合わせる。
    // **重なりは面の一部だけ**——ずらして置くので、接する矩形は小さい。
    out.push(Case {
        name: "box x box (faces coplanar, partial overlap)",
        why: "20 の箱を x に 20 ずらし、さらに y と z に 8 ずらす。側面 x=20 が同一平面で、接するのは 12 x 12 の矩形だけ",
        a: boxa.clone(),
        b: shifted(&boxa, 20.0, 8.0, 8.0),
        note: [
            "実測: 立体1つ（面で繋がる）",
            "実測: A がそのまま返る（中身は重なっていない）",
            "実測: 立体0つ（面は体積を持たない）",
        ],
    });

    // 点で触る（円錐どうし）。上向きの円錐2本を、底の縁が触れるまで寄せる。
    //
    // **上向きなので、触れるのは1点だけです**（母線ではありません。書いた
    // ときは「母線で接する」と考えていましたが、測る前の思い違いでした）。
    out.push(Case {
        name: "cone x cone (touching at one base point)",
        why: "底半径 10 の上向き円錐2本を x に 20 離す。底の縁が (10,0,0) の1点だけで触れる",
        a: cone.clone(),
        b: shifted(&cone, 20.0, 0.0, 0.0),
        note: [
            "実測: 立体が返る。**4-197 で通るようになりました**——8月30日の途中までは接触点をまたぐ長さ 5.3e-4 の破片で断られていた",
            "実測: 立体1つ、多様体。**メッシュの非多様体も 0 になりました**（4-197。8月30日の途中までは 6本）",
            "実測: 立体0つ。1点は体積を持たない",
        ],
    });
    ...
    out.push(Case {
        ...
        note: [
            ...
            "実測: 差 1981.297767（真値と一致）、**空洞1つを持つ立体**が返る。4-132 では体積が A + B（2207.49）になっていて検証ゲートが断っていたが、4-144（内側シェルの向き）で直った（4-147）",
            "実測: 球がそのまま返る",
        ],
    });
