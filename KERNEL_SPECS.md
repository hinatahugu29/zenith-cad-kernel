...
| **厚み付け (Thicken)** | `thicken` | 単一パッチ（平面 / NURBS / Coons）の法線方向オフセットと側面閉鎖による完全密閉ソリッド化。`thicken_shell` は複数面シェルを受け取り、**各面を個別に厚み付けして Union する**。**2026/09/08 に、その作りで正しいと測れました**（HANDOVER 4-390。`thicken_sheet_probe`）——**同一平面2枚 3600、直角に折れた2枚 3780、3枚のコの字 5580 が、いずれも包除の式と相対差 0**。**離れた2枚は断り、代わりの口（`boolean_solids_exact_result`）まで名乗る**。**足りなかったのは検体で、作りではなかった。** **曲がった板はまだ入れていない**——`thicken_face_with_samples` は刻みに依るので（8/16/24 標本で 8.27e-5 → 4.49e-6 → 8.25e-7）、閉じた式で 0 は出ない。 |
