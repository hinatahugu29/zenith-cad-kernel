...
- **`linkrods.step`（実物の STEP）は、まだ切れません。** 縫合で相手のいない
  ...
  測りました**（4-345、4-346）——**共有する稜 94 本のうち 28 本が、どちらの
  曲面にも乗っていません**（`screw.step` は 0 本）。**「正しい位置」が
  ファイルの中に無い**ので、**交わっているかどうかを、ファイルの精度では
  決められません**。**ここから先は「決める話」**です——直し方ではなく、
  **何を交わったことにするか**を決めることになります
- **読み込んだ `screw.step` の表示メッシュは、16・20・32・48 分割で壊れます**
  ——**2026/09/06 に、10 通りの刻みすべてで水密になりました**（`screw.step`・
  `linkrods.step` とも 0／0）。**1 つの刻みだけ見るのは測り方の穴**でした
...
——**幅を緩める手は 5 回とも外し**（4-323、4-327、4-328、4-339、4-343）、
**根は「共有する稜 28 本がどちらの曲面にも乗っていない」こと**でした
（4-345）。**交線とトリム境界は 1.1e-4 ですれ違い、それはファイル自身の
