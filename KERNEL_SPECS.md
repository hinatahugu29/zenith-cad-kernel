...
**文書バージョン**: v3.4.4 (4-101の純円錐・円錐台凸リム面取りを反映)
**最終更新日時**: 2026年8月26日
...
| **稜フィレット / 面取り** | `edge_blend` | 直線稜×平面2面、純直円柱・純円錐/円錐台の凸円周、貫通円筒穴口、**円筒ボス根元・段付き軸の小径側90度凹円周**、**平面肩×直円錐ボスの非直角凹円周**を、いずれもフィレット/面取りする。円錐系は厳密有理トーラス扇形/円錐台で、上下リム、真円錐の実頂点、OCC全周円、剛体配置、閉形式体積、STEP、mesh 4〜32を検証。非円形根元は明示拒否（HANDOVER 4-72、4-92〜4-101）。 |
...
| 何を測ったか | 結果 | 再現コマンド |
| :--- | :--- | :--- |
| ワークスペース全テスト | **102 バイナリ（doctest 込み）/ 589 テスト 100% 合格**（0 failed, 0 ignored） | `cargo test --release --workspace --exclude zenith_py` |
