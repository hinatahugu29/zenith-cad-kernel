...
> | 項目 | 実測 |
> | :--- | :--- |
> | ブーリアン | 自作立体・他カーネルの立体とも、軸に平行な切り手でも27度傾けた切り手でも**断るものは無し**（180演算、WRONG 0・PANIC 0）。**検証つきの口でも 180/180**（HANDOVER 4-67、4-68）。ただし自作立体どうしの45ケースでは**6件がエラーのまま**。内訳は `box × box`（45度回転）の差と積、`box × cylinder`（接線）の差、`box × sphere` の3演算——**「すべて接線配置」ではありません**（HANDOVER 3-1、実測 2026/08/24）。規約は決めました（接触は位相を作らない）が、実装はこれから |
