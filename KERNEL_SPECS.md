...
**文書バージョン**: v3.3.1 (v3.3.0の棚卸しに、4-87の境界ear・接触穴テッセレーション修正を反映)
**最終更新日時**: 2026年8月25日  
...
>
> **達成していること（すべて外部検証つき）**:
> - 出力用メッシュの完全閉多様体化。4〜32分割に加え 48〜256 でも open: 0, non-manifold: 0, degenerate: 0。**ただし検体は `mesh_watertight_probe` の立体です**——**ブーリアンが曲面を割った結果はそこに入っておらず、そちらには非多様体が残っています**（2026年8月25日の全配置実測で3件。修正前は5件。HANDOVER 3-N-2b、4-83〜4-87）。
...
| **稜フィレット / 面取り** | `edge_blend` | **「直線の稜 × 両側が平面」のみ**（`BlendSite::locate` が他を断ります）。有理2次円筒フィレット曲面および平面面取り。体積誤差 $< 10^{-11}$。**他カーネルから読んだ検体14件のうち12件は、丸められる稜が 0 本**——円柱の縁、段付き軸の肩、面取り箱の稜はいずれも対象外です。掛かるものは閉じた式と 1e-12〜1e-15 で一致するので、**誤りではなく守備範囲**（`foreign_edit_probe`、HANDOVER 4-72。広げるときの設計メモは 3-4b）。 |
...
| **完全閉テッセレーション** | `zenith_tess::stitched` | 構造格子規則性の維持により、`mesh_watertight_probe` の検体では、**測った全分割数（4〜256分割）で 100% 完全閉多様体メッシュ（穴・非多様体・退化三角形ゼロ）**。**ただし全立体で成り立つわけではありません**——**ブーリアンが曲面を割った結果には非多様体が残ります**（全配置実測3件、24分割で各3〜6本。修正前は5件・9〜126本。B-Rep のほうは稜がすべてちょうど2回使われており健全です。HANDOVER 3-N-2b、4-83〜4-87）。格子パッチ内では適応細分を掛けないため、弦誤差は分割数で制御する（32分割で体積差 1e-3 台）。 格子から落ちて earcut ＋ 適応細分へ行った面は `grid_fallback_probe` が検体ごとに数えます（**落ちても外から見えない**ため。HANDOVER 4-71）。 |
...
| 何を測ったか | 結果 | 再現コマンド |
| :--- | :--- | :--- |
| ワークスペース全テスト | **98 バイナリ（doctest 込み）/ 527 テスト 100% 合格**（0 failed, 0 ignored） | `cargo test --release --workspace --exclude zenith_py` |
...
| 常設プローブ | **34/34 が exit 0** | 上記手順書の一覧 |
