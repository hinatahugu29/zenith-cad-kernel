...
impl GearBuilder {
    ...
    /// # 形について、名前より正確なこと
    ///
    /// **歯形はインボリュートではありません。** 歯1つにつき「歯底左・歯先左・
    /// 歯先右・歯底右」の4点を取り、それを直線で結んだ多角形を押し出して
    /// います。噛み合いを解く用途には足りません。
    ///
    /// `pressure_angle_deg` は基礎円半径を通して**歯底半径の下限**にだけ効き、
    /// 歯面の傾きには効きません。
    ///
    /// `bore_radius` は**穴を開けません**。同じく歯底半径の下限に効くだけです
    /// （`bore_radius + 0.5 * module` より内側には歯底を置かない）。軸穴が要る
    /// なら、円柱との差で開けてください。
    ///
    /// 体積は多角形の面積 × 厚みで閉じています。`builder_audit` はその値と
    /// 突き合わせています。
