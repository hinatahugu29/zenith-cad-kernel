...
>
> **達成していること（すべて外部検証つき）**:
> - 出力用メッシュの完全閉多様体化。4〜32分割に加え 48〜256 でも open: 0, non-manifold: 0, degenerate: 0。**ただし検体は `mesh_watertight_probe` の立体です**——**ブーリアンが曲面を割った結果はそこに入っておらず、そちらには非多様体が残っています**（実測5件。HANDOVER 3-N-2b、4-83〜4-86）。
...
| **完全閉テッセレーション** | `zenith_tess::stitched` | 構造格子規則性の維持により、`mesh_watertight_probe` の検体では、**測った全分割数（4〜256分割）で 100% 完全閉多様体メッシュ（穴・非多様体・退化三角形ゼロ）**。**ただし全立体で成り立つわけではありません**——**ブーリアンが曲面を割った結果には非多様体が残ります**（実測5件、24分割で 9〜126 本。B-Rep のほうは稜がすべてちょうど2回使われており健全です。HANDOVER 3-N-2b、4-83〜4-86）。格子パッチ内では適応細分を掛けないため、弦誤差は分割数で制御する（32分割で体積差 1e-3 台）。 格子から落ちて earcut ＋ 適応細分へ行った面は `grid_fallback_probe` が検体ごとに数えます（**落ちても外から見えない**ため。HANDOVER 4-71）。 |
