...
**文書バージョン**: v3.4.3 (4-100の非直角円錐状ボス根元局所面取りを反映)
**最終更新日時**: 2026年8月26日
...
| **稜フィレット / 面取り** | `edge_blend` | 直線稜×平面2面、純直円柱の凸円周（フィレット/面取り）、純円錐/円錐台の凸円周、貫通円筒穴口（両操作）、**円筒ボス根元・段付き軸の小径側90度凹円周（両操作）**に加え、**平面肩×直円錐ボスの非直角凹円周（両操作）**を局所処理する。円錐根元は厳密有理トーラス扇形/円錐台で、閉形式体積、G1、剛体配置、STEP、mesh 4〜32、OpenCASCADE差4.721e-13で検証。非円形根元と純円錐の凸キャップ円周面取りは明示拒否（HANDOVER 4-72、4-92〜4-100）。 |
...
| 何を測ったか | 結果 | 再現コマンド |
| :--- | :--- | :--- |
| ワークスペース全テスト | **102 バイナリ（doctest 込み）/ 588 テスト 100% 合格**（0 failed, 0 ignored） | `cargo test --release --workspace --exclude zenith_py` |
...
| FreeCAD ヘッドレス相互検証 | **25/25 完全一致**（ゲート、不一致で非ゼロ終了） | `tools/freecad_cross_validate.py` |
