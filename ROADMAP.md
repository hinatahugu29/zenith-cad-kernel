...
> の `linkrods.step` だけです（2026/09/03 時点）。
...
| **H8** 読んだ立体（STEP）を切る | **`screw.step` は達成**（9/3。4-290。恒等式 2.848e-13 / 2.874e-13、表示メッシュも結果もメッシュ非多様体 0）。**`linkrods.step` は未達**——縫合で相手のいない稜 56 本で、**公差の問題ではない**ところまで測ってある（4-291） 。**9/3**: 返ってこない問題は解けた（300 秒超 → 132〜147 秒。4-294）。切り手が**必ず端の特徴に接していた**ので普通の置き方を既定にした。刻みを振る門を足し、**自作の立体は全刻みで水密**・**読んだ `screw.step` は 16/20/32/48 で壊れる**と分かった（4-296、4-297）|
...
**残っているのは `linkrods.step` の縫合**です（相手のいない稜 56 本）。
