...
| `foreign_edit_probe` | **ゲート**。他カーネルから読んだ立体15件に、フィレット・面取り・プッシュプル・面の計測を掛ける。**掛からないことは欠陥として数えない**（実装していない組み合わせがある）。数えるのは「掛かったのに答えが違う」ほう——壊れた立体、体積の符号が逆、閉じた式から外れる。純円柱の両操作、純円錐/円錐台、穴口の両操作、段付き軸根元の両操作を閉じた式で採点し、2026/08/26時点で **18操作適用・閉形式20件・WRONG 0**（4-92〜4-98）。 |
