...
| `ZENITH_SNAP_WHY=1`（環境変数） | **切り込みの端を境界へ吸着した量**を1行ずつ出す（4-215）。端の制御点は曲線の端そのものなので、**動かした量ちょうど曲面から浮きます**（実測 1:1）。`Face::pcurves` が乗っていると認めるのは**絶対 1e-5** までなので、**1e-5 を超えた吸着はその片の p-curve を取れなくします**——そこから先は面積の検算が「測れなかった」と言って断ります。**許容の噛み合わせを直す前に、まずここで分布を取ってください** |
