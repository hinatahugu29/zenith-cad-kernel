...
fn test_regular_prisms_match_analytic_volume() {
    ...
    let hex_prism =
        PrimitiveBuilder::make_regular_prism(6, 10.0, 30.0).expect("make_regular_prism 6");
    assert!(
        hex_prism.is_topologically_valid(&tol),
        "hex prism must be valid closed solid"
    );
    ...
    let oct_prism =
        PrimitiveBuilder::make_regular_prism(8, 12.0, 20.0).expect("make_regular_prism 8");
    assert!(
        oct_prism.is_topologically_valid(&tol),
        "oct prism must be valid closed solid"
    );
    ...
    let tri_prism =
        PrimitiveBuilder::make_regular_prism(3, 15.0, 25.0).expect("make_regular_prism 3");
    assert!(
        tri_prism.is_topologically_valid(&tol),
        "tri prism must be valid closed solid"
    );
