...
| `ZENITH_REFINE_WHY=1`（環境変数） | **診断**。適応細分のあとに、**溶接距離の中へ入る頂点対**を「境界どうし / 境界と細分 / 細分どうし」に分けて数える。束ねられると三角形が潰れ、`weld` がそれを外し、跡が穴になる。**どこで作られた頂点どうしなのかで直す場所が変わる**ので、そこを分ける。実測では**すべて細分どうし**で、境界の点は1組も関与していなかった（HANDOVER 4-123）。「作らずに置いた中点」の数もここに出る |
