...
### 3-3. ブーリアン: 曲面同士の交差（SSI）★次はこれ

3-1 と 3-2 が済んだので、これが最優先です。なお 3-2 で最近傍点探索
（`ExtremumEngine::point_to_surface`）が継ぎ目と退化行を越えられるようになり、
総当たりと 4e-9 で一致するようになりました。SSI の Newton marching は同じ
探索の上に乗るので、以前より足場は確かです。
