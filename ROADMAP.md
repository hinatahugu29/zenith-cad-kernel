...
> | エッジフィレット / 面取り | 直線稜×平面2面、純直円柱の凸円周（フィレット/面取り）、純円錐/円錐台の凸円周に加え、**貫通円筒穴口の局所フィレット**を処理。読んだ15検体で対象稜0は **12→8件**。ボス根元・段付き軸肩と円錐/穴口面取りはまだ対象外（HANDOVER 4-72、4-92〜4-95） |
>
> **「実用に耐えるには何が要るか」は [`HANDOVER.md`](HANDOVER.md) の9章に
> あります。** 性能（`torus × box` の積で solve 38.5秒）、フィレットの守備範囲
> （貫通穴口まで対応済みだが、ボス根元・段付き軸肩は未対応）、2Dスケッチの欠落、そして
