...
> | スライサーでエラーが一切起きない | **測っていません。** 測ってあるのは「**自分で作った立体は** 4〜64 分割の 11 通りすべてで open 0・非多様体 0・退化 0」（`mesh_watertight_probe`。2026/09/06 に**門にしました**——それまでは**開いていても終了コード 0** でした。4-348。`mesh_density_probe` でも 10 通りの刻みで確かめてあります。4-297）です。**読み込んだファイルも、いまは 10 通りすべてで水密です**——`screw.step` は 16・20・32・48 分割で壊れていましたが、2026/09/06 に **0** になりました（4-296、4-329、4-332、4-338、4-347）。スライサーに掛けた記録はありません |
