...
> | **できないこと** | 実物の `linkrods.step` はまだ切れない（縫合で相手のいない稜が 47／47／40 本。**非多様体は 0**）。**根は測ってあります**——**ファイルの精度では交わっているかどうかが決まりません**（交線とトリム境界のすれ違い 1.1e-4 に対し、ファイルの粗さ 2.4e-4。4-346）。**「共有する稜 28 本が乗っていない」と書いていましたが取り下げます**——**1e-9 で測っていたためで、面が申告する粗さでは 0 本**です（4-351）。**幅の話でもありません**——申告した粗さを縫合に使っても 1 本も動かず、**相手までの距離は 0 か 0.01 以上で間が空**です（4-291、4-352） |
