...
fn non_manifold_vertices(shell: &Shell, tol: &Tolerance) -> usize {
    ...
    let key_of =
        |point: Point3, points: &mut Vec<Point3>, js: &mut Vec<Vec<(u64, u64)>>| -> usize {
            if let Some(i) = points
                .iter()
                .position(|p| (p - point).norm() <= tol.linear * 10.0)
            {
                return i;
            }
            points.push(point);
...
fn report(name: &str, solid: &Solid) {
    ...
    println!(
        "{name:32} non_manifold_vertices={}",
        non_manifold_vertices(&solid.outer_shell, &tol)
    );
}
...
fn main() {
    ...
    report(
        "cylinder",
        &PrimitiveBuilder::make_cylinder(10.0, 30.0).unwrap(),
    );
    report("sphere", &PrimitiveBuilder::make_sphere(15.0).unwrap());
    report(
        "cone",
        &PrimitiveBuilder::make_cone(10.0, 0.0, 20.0).unwrap(),
    );
    report(
        "frustum",
    ...
    report(
        "drilled",
        &zenith_algo::HoleBuilder::make_drilled_box(30.0, 30.0, 15.0, 5.0).unwrap(),
    );
    report(
        "hollow",
        &zenith_algo::ShellBuilder::make_hollow_box(30.0, 40.0, 25.0, 2.5, 1).unwrap(),
    );
    report(
        "filleted",
        &zenith_algo::FilletBuilder::fillet_box_z_edges(20.0, 30.0, 40.0, 4.0, &tol).unwrap(),
    );
    report(
        "chamfered",
        &zenith_algo::ChamferBuilder::chamfer_box_z_edges(20.0, 30.0, 40.0, 3.0, &tol).unwrap(),
    );
    ...
    let r = BooleanEngine::boolean_solids_exact_result(&cube, &corner, BooleanOpType::Union, &tol)
        .unwrap();
    report("corner-touching union", &r.solids[0]);
    let face = BrepTransform::translate_solid(&cube, Vec3::new(10.0, 0.0, 0.0));
    let r2 = BooleanEngine::boolean_solids_exact_result(&cube, &face, BooleanOpType::Union, &tol)
        .unwrap();
