...
- **`linkrods.step`（実物の STEP）は、まだ切れません。** 縫合で相手のいない
  稜が **和 47 / 差 47 / 積 40 本**残ります（2026/09/05 実測）。**入口は
  名指しできています**（4-306〜4-319）が、直っていません
- **読み込んだ `screw.step` の表示メッシュは、16・20・32・48 分割で壊れます**
  ——正しくは **24 と 48 だけ**です（穴 4／10、重なり 2／5。2026/09/05 実測。
  **8・12・16・20・32 は水密**）。**1 つの刻みだけ見るのは測り方の穴**でした
...
（**24 と 48 だけ**。穴 4／10、重なり 2／5。2026/09/05 実測。4-296、4-329、4-332）。**利用者は刻みを自分で選ぶ**ので、
