...
| **H8** 読んだ立体（STEP）を切る | **着手済み・未達**。実物2ファイルが読めて切れるが、恒等式の破れ 0 には届いていない |

**次は H8 です。** いま止まっているのは速度で、`linkrods.step` の演算が
返りません。**仕事量で測ると、射影の 9 割が p-curve の導出**でした
（4-270、4-271）。**壁時計では「交線を辿るのが重い」と誤読します**——
実測では辿りは 30 秒で終わっています。
...
| STEP インポーター | **着手済み**（2026/09/02）。OCCT が配る実物の `screw.step`・`linkrods.step` が**読めて切れます**。円錐の頂点の向こう側（4-261）、紡錘トーラスの内枝（4-264）を直し、**面が自分の粗さを持ち歩く**ようにしました（4-266）。残るのは速度と非多様体（4-269〜4-271） |
