    ...
    match r {
        Ok(Ok(res)) => {
            ...
            let v: f64 = res
                .solids
                .iter()
                .map(|s| zenith_algo::MassCalculator::compute_from_brep(s, &params).volume)
                .sum();
            println!(
                "{name:38} ok solids={} all_valid={all_valid} V={v:.3}",
                res.len()
            );
        }
        Ok(Err(e)) => println!(
            "{name:38} rejected: {}",
            e.chars().take(44).collect::<String>()
        ),
...
fn main() {
    ...
    let inner = BrepTransform::translate_solid(
        &PrimitiveBuilder::make_box(2.0, 2.0, 2.0).unwrap(),
        Vec3::new(4.0, 4.0, 4.0),
    );

    for (name, b) in [
        ("self", &cube),
        ("half overlap", &shifted),
        ("face touching", &touching),
        ("edge touching", &edge_touch),
        ("corner touching", &corner_touch),
        ("sliver overlap", &sliver),
        ("disjoint", &far),
        ("contained", &inner),
    ] {
        for (opname, op) in [
            ("union", BooleanOpType::Union),
            ("inter", BooleanOpType::Intersection),
            ("diff", BooleanOpType::Difference),
        ] {
