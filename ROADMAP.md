...
> | エッジフィレット / 面取り | 直線稜×平面2面、純直円柱の凸円周（フィレット/面取り）に加え、**純円錐/円錐台の凸円周フィレット**を厳密トーラス扇形で処理。読んだ14検体で対象稜0は **12→9件**。穴・ボス・段付き軸等の複合立体と円錐円周面取りはまだ対象外（HANDOVER 4-72、4-92〜4-94） |
>
> **「実用に耐えるには何が要るか」は [`HANDOVER.md`](HANDOVER.md) の9章に
> あります。** 性能（`torus × box` の積で solve 38.5秒）、フィレットの守備範囲
> （純円柱は対応済みだが、複合立体の穴・ボス・段付き軸は未対応）、2Dスケッチの欠落、そして
