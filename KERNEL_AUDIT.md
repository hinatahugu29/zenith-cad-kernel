...
7. Extend exact boolean coverage beyond axis-aligned cases. Plane/cylinder support intersection still only recognises planes perpendicular or parallel to the cylinder axis, cylinder patches must be Z-axis aligned, NURBS/NURBS pairs are unsupported, and a plane that cuts one patch twice is rejected rather than producing two rulings.
