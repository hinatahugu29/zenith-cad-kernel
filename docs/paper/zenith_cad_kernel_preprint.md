...
2. **表示メッシュの水密性の測定範囲**:
   読み込んだ実物 STEP 2 件は 2026/09/06 に**測った 10 通りの分割数すべてで水密**になった（§5.2）が、これは**常設検体の実測範囲**であって分割非依存性の証明ではない。**単一の分割数での測定は健全性の証明にならない**ため、分割を振る掃き出しを門として常設している（自作立体は 4〜64 の 11 通り、読み込みファイルは 6〜64 の 10 通り）。
