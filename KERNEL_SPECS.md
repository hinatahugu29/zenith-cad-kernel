...
**文書バージョン**: v2.3.0 (他カーネルの解析曲面読み込み・p-curve と最近傍点探索の修正・実測ベース改訂版)  
**最終更新日時**: 2026年8月20日  
...
```mermaid
graph TD
    A[Zenith CAD Kernel Core v2.3.0] --> B[1. 数値幾何・自由曲面エンジン]
    A --> C[2. B-Rep トポロジー構造]
...
`step_import_test` / `foreign_analytic_surface_test` / `pcurve_fidelity_test`）、`cargo test` で常時検証されます。
現在 38 テストバイナリ・268 テストがすべてグリーンです。
