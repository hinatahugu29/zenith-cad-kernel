...
**トーラス × ボックスに残っている最後の1つ: 円環のキャップ**

envelope は 28/45 のままです。段階ごとの実測はこうなっています。

```
edges 8, applied 8, skipped 0, pieces 8..16, caps 2, cap loops 2,
unmatched 8, non-manifold 0, same-dir 8
```

交線も分割も通っていて、**縫合だけが合いません**。理由は、平面 z=-2 が
トーラスを切った断面が**円環**（外周 r=15.46、内周 r=8.54）なのに、
`build_planar_caps_from_intersection_edges` が**ループ1本につき円板1枚**を
作っているためです。円環を2枚の独立した円板として塞ぐと、辺が余ります。

やること: 同じ平面に乗るループ同士を入れ子で判定し（平面の2D枠に落として
内外判定）、外側のループを外周、内側を穴として**1枚の面**にする。
`CapBuilder::make_planar_cap` は外周ワイヤ1本しか受け取らないので、
穴付きを作れるようにする必要があります。
