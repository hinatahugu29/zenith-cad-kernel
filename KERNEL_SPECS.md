...
| **CSGブーリアン演算** | `BooleanEngine` | Union（結合）、Difference（差分）、Intersection（交差）。**対応範囲は限定的で、範囲外は誤答ではなくエラーを返す**。実測36ケース中16が成功し、誤答はゼロ。対応済みは軸平行ボックス同士、**円柱による貫通穴・止まり穴（任意軸）**、離れた立体の和（複数ソリッド結果）。未対応は曲面同士の交差（円柱×円柱、球×球など）。詳細は `cargo run -p zenith_algo --example boolean_envelope` で随時測定できる。 |
