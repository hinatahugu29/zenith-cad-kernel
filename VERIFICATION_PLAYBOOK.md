...
| `contact_placement_probe` | **B-Rep ゲート**。接触している**30配置・90演算**を測る（2026/08/28 実測: **81件が立体を返し、9件を断る。B-Rep 非多様体 0、mesh 非多様体は 1演算**。HANDOVER 4-116〜4-133）。**通しで走らせた3回が1バイトも違いません**——2026/08/27 まで、ここは実行ごとに1演算が揺れていました（4-132）。**新しい数字を書く前に2回測って突き合わせてください。** **exit 1 にするのは B-Rep 側だけです**——mesh 側は数えて印字するだけで、ラチェットは掛かっていません（掛けると常設ゲートが赤で止まります）。断られること自体は赤にしない。答えが本当に非多様体なので断るものと、未実装で断るものがある。`ZENITH_CONTACT_FILTER` と `ZENITH_CONTACT_OP` で単一配置・演算へ絞れる。`ZENITH_REFUSE_WHY=1` で断った理由、`ZENITH_SPLIT_WHY=1` で割れなかった理由、`ZENITH_FACE_OWNER_WHY=1` で非多様体の稜を出した面、`ZENITH_TESS_WHY=1` で ring・earcut 各段・weld が外した枚数を出す。 |
