...
- **`linkrods.step`（実物の STEP）は、まだ切れません。** 縫合で相手のいない
  ...
  **残っているのは、名前の付いた「まだ書いていない」の山**でした（4-358）
  ——**9 枚の面が交線を受け取って 1 本も使えておらず**、断り文は
  **「交線の端が境界に届いていない」87**、**「円柱の側面しか割れない」60**、
  **「輪が 2 辺以下」42**、**「穴のある NURBS 面は未実装」12**（のべ）。
  **謎ではなく、書いていない機能です**
...
のべも同数です。合計 134 本）。**非多様体は 0 本**です——9/4 の一時期は
