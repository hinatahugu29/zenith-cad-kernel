...
1. **トポロジー縫合における割り方の整合（Stitching Alignment）**:
   複雑な多面アセンブリ（例: `linkrods.step`）において、隣接面同士のトリム境界の分割数や端点接続が不揃いになる問題（相手のいない稜の発生）の解消。**現状の未達量は 3演算で 47／47／40 本**（非多様体 0 本。2026/09/06 実測）。**原因は計測済みである**——共有する稜 94 本のうち **28 本がどちらの支持曲面にも乗っておらず**（`screw.step` は 0 本）、交差曲線とトリム境界は **1.1×10⁻⁴** ですれ違う。これは**ファイル自身が申告する不確かさ 2.4×10⁻⁴ を下回る**ため、**交差の有無はファイルの精度では決定できない**。したがって残る選択は「粗さの範囲内のすれ違いを交差とみなす（＝ファイルに書かれていない仮定を置く）」か「拒否を継続する」かの**設計判断**であり、公差調整で解ける問題ではない（幅を緩める試行は 5 回とも悪化した）。
2. **表示メッシュの水密性の測定範囲**:
