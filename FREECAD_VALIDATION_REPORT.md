...
> **2026年8月28日 追記（4-133〜4-142。外部の物差しとは別の掃き出し）**
>
> **外部カーネルのゲートは全部そのままです**（相互検証 27/27、ショーケース
> 54/54、書き戻し 7/7、非STEP出力 8/8）。**それでも誤答が3件ありました。**
>
> 閉じた式と恒等式で総当たりに掃いたところ、**検証ゲートも FreeCAD の
> ゲートも通り抜ける誤答**が出ました——同じ立体どうしの和が体積2倍、
> 裏返しの立体が打ち消し合って検査を通る、二重被覆が検査を3つとも通る。
> 本番経路のパニックも1件（空洞のある立体で添字が範囲外）。**どれも
> 直してあります**（HANDOVER 4-133〜4-141）。
>
> **外部の物差しが緑でも、内部の不変量が破れていることがあります。**
> FreeCAD が読めるのは「書き出した1つの立体」で、`A ∪ B` と `A ∩ B` の
> **関係**は見ていないからです。掃いた軸と結果は本文末尾の表に置きました。
>
> **その誤答（`cylinder` × `elliptic_prism`）は同じ 2026/08/28 に解決しました**
> （`earcut` の出力を面積で検算していなかった。4-142、4-143）。2026/08/30 の
> 再測定でも恒等式の破れ 0・残差の最悪 7.052e-10 です。

> **2026年9月22日 追記（4-116 から 4-508 のあと、25 日ぶりに掛け直し）**
>
> **外部の物差しを 4 本とも掛け直しました。終了コードは全部 0 です。**
>
> | ゲート | 2026/08/27 | **2026/09/22** |
> | :--- | :--- | :--- |
> | `freecad_cross_validate.py` | 27/27 | **29/29**（検体が 2 つ増え、全部一致） |
> | `verify_showcase.py` | 54/54 | **54/54** |
> | `verify_reexport.py` | 7/7 | **7/7**（1e-8 以内） |
> | `verify_mesh_exports.py` | 8/8 | **8/8** |
>
> **残差はほとんどが 1e-11 〜 1e-16。** 目立つのは `swept_pipe`（体積
> 8.95e-8、面積 3.63e-7）、`helix_spring`（2.01e-8 / 4.39e-8）、
> **`spur_gear_m2_z18`（体積 7.18e-5）**——**歯車は記録どおり、
> OpenCASCADE 側の `shape.Volume` の話**です（面積は 8.2e-8 で一致）。
>
> **この間に動いたもの**: テッセレーション、ブーリアン、面の分割、
> 内外の判定、交線の出し方（4-116 〜 4-508）。**それでも外の数は同じ**です。
...
STEP 経由の相互運用について、**29 対象すべてが OpenCASCADE で Solid・`isValid`・`isClosed` として読まれ、
体積・表面積・断面積が両カーネルで一致**します（**2026/09/08 実測**。23 → 27 → 29 と検体が増えています）。

**2026/09/08 に 2 検体を足しました**（HANDOVER 4-407）——`sketch_plate_with_hole` と
`sketch_revolved_ring`。**スケッチから作った立体**で、**穴が最初から内側の輪として入り、
円弧が輪の一部として混ざる位相**です。**それまでの 27 検体は、全部ビルダーのプリミティブか
その組み合わせ**でした。残差は **9.427e-13 / 4.444e-13** と **1.088e-11 / 1.490e-11**。
...
（`BooleanResultVerifier`）が入っており、`contact_placement_probe` の **47 配置・141 演算**では
**135 成功・6 拒否・非多様体を返したもの 0** です（2026/09/03 実測。8月30日は
35配置・105演算で 99 / 6、8月28日は 30配置・90演算で 81 / 9。HANDOVER 4-169〜4-226）。
**拒否のうち「未実装だから」は 0 件**で、そこは赤ゲートになりました（4-226）。拒否の 6 件は**すべて
「答えが本当に非多様体だから」**で、場所を名指しします。**この門は
2026/08/30 から恒等式も測ります**——**41配置**で破れ 0、残差の最悪 3.900e-8
（2026/09/04 実測。8/30 は 29 配置でした）。

## 不変量による総当たりの掃き出し（2026/08/28）

外部の物差し（FreeCAD / OpenCASCADE）とは別に、**閉じた式と恒等式**で総当たりに
掃きました。**検証ゲートを通り抜ける誤答が3件と、本番経路のパニックが1件**
出ました。いずれも直してあります（HANDOVER 4-133〜4-141）。

| 掃いた軸 | 通数 | 結果 |
| :--- | ---: | :--- |
| 中に入っている（4形 × 4形 × 3演算） | 48 | 48/48 |
| 離れている（5形 × 3演算） | 15 | 15/15 |
| まったく同じ立体どうし（5形 × 3演算） | 15 | 13/15（残り2件は断り） |
| 公差の前後だけずらす（2形 × 5段 × 3演算） | 30 | 誤答 0（5件は断り） |
| 半分重ね（4配置 × 3演算） | 12 | 12/12 |
| 自分の軸まわりに回して重ねる（4形 × 5角度 × 3演算） | 60 | 60/60 |
| 桁の離れたスケール（5段 × 3演算） | 15 | 15/15 |
| 入れ子3段 | 3 | 3/3 |
| 引数の順序（34配置 × 2演算） | 68 | 68/68 |
| 結合則（4組 × 2演算） | 8 | 食い違い 0 |
| 面の一部だけが接する（5配置 × 3演算） | 15 | 15/15 |
| 同軸で一部だけ重なる（4配置 × 3演算） | 12 | 12/12 |
| **他カーネルの立体どうし**（10検体・45組） | 45 | **当時 誤答 1**（下記。2026/08/28 に解決） |

**この誤答は解決しました（2026/08/28）。** OpenCASCADE が書いた `cylinder` と
`elliptic_prism` を中心を揃えて重ねると、**和と差が真値より 150.951 大きく**
なっていました（積は正しい。相対 1.2%）。分割数を 24 / 48 / 96 と変えても値が
1ビットも動かないことから、テッセレーションではなく幾何の誤りと分かりました。
原因は `earcut` の出力を面積で検算していなかったことで、恒等式の残差は
1.2e-2 → **9.234e-10** になりました（HANDOVER 4-142、4-143）。2026/08/30 の
再測定は 15組・恒等式の破れ 0・残差の最悪 **7.052e-10** です。

**検証ゲートも内外一貫性も通り、恒等式でしか見えなかった**という事実のほうは
残ります。上の表を「45 → 誤答 1」のまま置いてあるのは、**外部の物差しが緑でも
内部の不変量が破れうる**ことの記録だからです。

これらは常設テスト 10 本として残してあります（`boolean_containment_matrix_test`
ほか）。
