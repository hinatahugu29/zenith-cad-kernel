...
>
> **達成していること（すべて外部検証つき）**:
> - 出力用メッシュの完全閉多様体化。4〜32分割に加え48〜256でも open: 0, non-manifold: 0, degenerate: 0。ブーリアン曲面分割を含む `contact_placement_probe` は **30配置・90演算（うち81演算が立体を返す）で B-Rep 異常 0**。**mesh は 1演算だけ残っています**（`sphere × cylinder (eccentric)` の差、13本。2026/08/28 実測、HANDOVER 4-116〜4-142）。**この文書は長らく「7配置・21演算で B-Rep / mesh とも異常0、赤ゲートへ昇格」と書いていましたが、実装と違っていました**——赤ゲート（exit 1）は B-Rep 側だけで、mesh 側は数えて印字するだけです（HANDOVER 4-116）。mesh の異常は着手前の 11演算・最大165本から 1演算・13本まで落ちました（4-116〜4-131）。**これは常設検体の実測範囲で、任意の全立体の証明ではありません。**
...
| **完全閉テッセレーション** | `zenith_tess::stitched` | 構造格子規則性の維持により、`mesh_watertight_probe` の検体では、**測った全分割数（4〜256分割）で100%完全閉多様体メッシュ（穴・非多様体・退化三角形ゼロ）**。ブーリアン曲面分割を含む `contact_placement_probe` は **30配置・90演算で B-Rep 異常 0**、**exit 1 にするのは B-Rep 側だけ**です（mesh 側は数えて印字するだけで、ラチェットは掛かっていません。HANDOVER 4-116）。**mesh の非多様体は 1演算だけ残っています**（13本。着手前は 11演算・最大165本。4-116〜4-131）。**任意の全立体を証明するものではなく常設検体の実測範囲**です。格子パッチ内では適応細分を掛けないため、弦誤差は分割数で制御する（32分割で体積差1e-3台）。格子から落ちて earcut＋適応細分へ行った面は `grid_fallback_probe` が検体ごとに数えます。 |
