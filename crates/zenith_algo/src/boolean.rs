...
impl BooleanEngine {
    ...
    ///
    /// # ⚠ **これは表示用です。体積を読まないでください**（4-421）
    ///
    /// **三角形を割りません。** **重心が相手の内側かどうかで、
    /// 三角形を丸ごと採るか捨てるか**を決めます。**境界をまたぐ
    /// 三角形は、丸ごと採られるか丸ごと捨てられます。**
    ///
    /// **閉じた立体は返りますが、大きさが違います**——実測（4-421。
    /// 刻み 8、`20x20x20` と `10x10x h`）:
    ///
    /// ```text
    /// b の高さ   演算   閉じた式    返る値
    ///   10       和      8000      8000      （たまたま合う）
    ///   20       和      8000      8666.667  （+8.3%）
    ///   20       積      2000      1333.333  （-33%）
    ///   30       積      2000      1000      （-50%）
    /// ```
    ///
    /// **高さ 10 のときだけ合うのは、面がちょうど三角形の境目に
    /// 乗るから**です。**「合っている例がある」は、正しいことの
    /// 証明になりません。**
    ///
    /// **正しい答えが要るなら**——`boolean_solids_exact`（B-Rep）か、
    /// Python なら `make_exact_box_boolean` / `make_exact_drill_boolean`
    /// を使ってください。**そちらは閉じた式とぴったり合います**
    /// （4-421 で 3 演算とも 1e-6 以内）。
