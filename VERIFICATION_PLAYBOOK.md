...
| `contact_placement_probe` | **ゲート**。接触している配置を演算ごとに測る（2026/08/25 時点で **11件が立体を返し、1件を名指しして断る。非多様体を返したもの 0**）。**「非多様体のはずだ」と決めつけず、返ってきた立体のメッシュで稜がちょうど2枚に共有されているかを数える。** 断られること自体は赤にしない。**断り方は2種類あります**——答えが本当に非多様体で断るもの（円柱の接線の差。場所を名指しします）と、まだ実装していないもの（球の極）。赤にするのは**非多様体を立体として返した**ときだけ。同じ4面接触を、切る平面が球の極を通る向きと通らない向きの両方で持っているので、**接触のせいか極のせいかが1枚で分かる**（HANDOVER 3-1） |
