...
impl IntersectionMarcher {
    ...
    pub fn find_seeds(
        ...
        for (_, index_a, index_b) in pairs.iter().take(limit * 8) {
            ...
            // **接線に近すぎる所から歩き始めても、交線のどちらへ進むのかが
            // 決まらない**ので、ここで種を捨てます。**余裕は 10 倍**
            // （つまり 1e-3）で、**`land_on_curve` の 1 倍より 1 桁きつい**。
            //
            // **この 10 倍を測りました**（4-414。`seam_torus_wall_probe`）。
            // **倍率を 10 → 3 → 1 → 0.3 → 0.1 と下げても、交線 14 /
            // 面片 33 / 相手のいない稜 12 / 非多様体 3 が 1 つも動きません。**
            // **ここは、足りない交線を捨てていません。** 測る口は
            // **残していません**——**動かないと分かった口は、読む人の
            // 邪魔にしかなりません**（4-412 と同じ扱い）。
