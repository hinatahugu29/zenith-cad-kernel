...
> | エッジフィレット / 面取り | 直線稜×平面2面、純直円柱の凸円周（両操作）、純円錐/円錐台の凸円周、貫通円筒穴口（両操作）、**円筒ボス根元・段付き軸の小径側90度凹円周（両操作）**、**平面肩×直円錐ボスの非直角凹円周（両操作）**を処理。読んだ15検体で対象稜0は **12→7件**。非円形根元と純円錐の凸キャップ円周面取りはまだ対象外（HANDOVER 4-72、4-92〜4-100） |
>
