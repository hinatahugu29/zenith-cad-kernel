...
fn cases() -> Vec<Case> {
    ...
    out.push(Case {
        ...
        note: [
            "実測: **断られる（穴）**。触れているのは1点だけなので、規約どおりなら**立体2つ**が返るはずです（`box x sphere (touching at one point)` と同じ）。原因は接触点をまたぐ長さ 5.3e-4 の破片（4-194。未解決）",
            "実測: 立体1つ。**ただしメッシュが非多様体（6本）**——同じ破片が出ています（4-194）",
            "実測: 立体0つ。1点は体積を持たない",
        ],
    });
