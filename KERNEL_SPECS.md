...
**最終更新日時**: 2026年9月7日（**2026/09/16 に読み直しました**——
本文の数字はどれも日付つきで、**古くなっているものはありません**。
**現在地点の数字は [`HANDOVER.md`](HANDOVER.md) が正**です）
**ステータス**: 完全自前 Rust B-Rep エンジン。**本書の数値はすべて実測値**。

> **2026/09/16 時点の実測**（この文書の数字ではなく、いまの値）:
> 通しテスト **149 バイナリ / 738 件通過 / 失敗 0 / 警告 0**
> （`cargo test --release --workspace --exclude zenith_py`）、
> 門 **50 本すべて緑**（`bash tools/run_gates.sh --quick` の `== 門 ==`。
> **うち 6 本は外部ファイルが無く回っていません**——
