...
Face orientation validation was planar-only, which is why an inside-out cylindrical wall could pass. It now covers NURBS faces under the same rule - the outer trim loop's winding in UV must agree with the face's orientation - skipping loops that only run along a seam and enclose no UV area, as the single-face sphere and torus do. Checking every primitive and operation against that rule found the drilled box again: the hole's wall wire and both caps' inner loops ran the same way round as the outer loops, so the hole was not wound as a hole. They were consistent with each other, which is why edge pairing passed; they were just consistently backwards. Reversing them makes every face in every shape the kernel builds pass the rule.

Both of those defects slipped past `validate_boundary_on_surface()`, which sampled loops through `Wire::sample_points()`. That shortcuts a linear edge to its two endpoints - right for display, but it means a chord drawn straight across a curved face is only ever tested where it touches the surface. Boundary validation now samples every edge at interior parameters too, and a chord across a cylinder is reported off-surface by the 2.9 mm it actually deviates.

Turning that on immediately caught a third defect: `sweep_circle_along_curve()` built each swept patch's seam edges with `Edge::line_between()`, so a pipe along a curved path had straight seams that left the swept surface entirely. They are now the surface's own `u = const` iso-curves. A pipe along a straight path was unaffected, which is why nothing noticed.
