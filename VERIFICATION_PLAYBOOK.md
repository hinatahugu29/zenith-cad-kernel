...
> 「168 バイナリ」には**プローブ（examples、73本）が入る**ので、被覆が
...
**最終確認**: 2026年8月26日（HANDOVER 4-96 まで）
...
**期待**: 101 テストバイナリ（doctest 込み）、575 テスト、失敗 0。
...
**期待**: `19 of 19 subjects agree across both kernels`、**終了コード 0**。
...
| `contact_placement_probe` | **B-Rep＋meshゲート**。接触している7配置・21演算を測る（2026/08/25時点で **19件が立体を返し、2件を断る。B-Rep非多様体0、mesh非多様体0**。HANDOVER 4-89）。**B-Repの稜使用数と派生meshの三角形辺使用数を分けて数え、どちらかが1本でも非多様体ならexit 1。** 断られること自体は赤にしない。答えが本当に非多様体なので断るものと、未実装で断るものがある。`ZENITH_CONTACT_FILTER` と `ZENITH_CONTACT_OP` で単一配置・演算へ絞れ、`ZENITH_TESS_WHY=1` でring、earcut各段階、残るmesh edgeを表示する。 |
...
| `foreign_edit_probe` | **ゲート**。他カーネルから読んだ立体15件に、フィレット・面取り・プッシュプル・面の計測を掛ける。**掛からないことは欠陥として数えない**（実装していない組み合わせがある）。数えるのは「掛かったのに答えが違う」ほう——壊れた立体、体積が増える、閉じた式から外れる。純円柱の両操作、純円錐/円錐台、複数穴プレートと回転リングの穴口フィレット/面取りを閉じた式で採点し、2026/08/26時点で **16操作適用・閉形式18件・WRONG 0**（4-92〜4-96）。 |
...
`crates/zenith_algo/examples/` には 72 本の実行ファイルがあり、**CI に
...
| `target/validation/` | 相互検証用21形状＋OCC が書いた参照ファイル | `freecad_cross_validate.py`（21/21のゲート） |
