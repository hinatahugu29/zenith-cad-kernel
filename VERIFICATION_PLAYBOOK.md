...
**最終確認**: 2026年8月25日（HANDOVER 4-92 まで）
...
**期待**: 99 テストバイナリ（doctest 込み）、543 テスト、失敗 0。
...
**期待**: `16 of 16 subjects agree across both kernels`、**終了コード 0**。
...
| `foreign_edit_probe` | **ゲート**。他カーネルから読んだ立体14件に、フィレット・面取り・プッシュプル・面の計測を掛ける。**掛からないことは欠陥として数えない**（実装していない組み合わせがある）。数えるのは「掛かったのに答えが違う」ほう——壊れた立体、体積が増える、閉じた式から外れる。純円柱の円形キャップ稜も閉じた式で採点し、2026/08/25時点で **9操作適用・閉形式11件・WRONG 0**（4-92）。 |
...
| `target/validation/` | 相互検証用16形状＋OCC が書いた参照ファイル | `freecad_cross_validate.py`（ゲート） |
