...
| `ZENITH_DELAUNAY_FLIP=1`（環境変数） | **未完成の手を開く口**（9-H の H5）。細分の前に、内側の対角線を Delaunay へ入れ替える。1面あたり最大 14,996 → **7,646 枚**で H5 に届くが、**表示メッシュが3件壊れる**（4-209）。壊れるのは入れ替えのせいではなく、面をまたいで境界の点が食い違っているところが露わになるため。**既定では入りません** |
