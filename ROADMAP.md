...
| STEP インポーター | **着手済み**（2026/09/02〜03）。OCCT が配る実物の `screw.step`・`linkrods.step` が**読めて切れます**。**表示メッシュは2ファイルとも水密**（穴 326／1330 → **0／0**。4-286、4-288）。**`screw.step` は恒等式が閉じます**（**3.845e-13 / 3.899e-13**。2026/09/05 実測。4-290）。**表示メッシュは 2026/09/06 に 10 通りの分割数すべてで水密になりました**（4-296、4-329、4-332、4-338、4-347。破れの合計 472 → 147 → 21 → **0**）。円錐の頂点の向こう側（4-261）、紡錘トーラスの内枝（4-264）、面が自分の粗さを持ち歩く（4-266）、継ぎ目に混ざって乗る区間（4-288）を直しました。残るのは `linkrods.step` の縫合——**相手のいない稜 47／47／40、非多様体 0**（2026/09/05 実測。4-326） |
