...
fn main() {
    ...
    //
    // **いまは 0 件です**（4-500）。**12 行すべてが 3 演算とも返ります。**
    //
    // **6 → 3 は 4-498**（切れたときだけ辿り直す ＋ 受け入れ幅を場面に乗せる）、
    // **3 → 0 は 4-500**（**軸に平行な平面 × 円錐を、辿らずに双曲線で出す**）。
    // **尖った円錐の 3 枚の壁は、どれも辿ることから来ていました**
    // ——**辿らなければ、当たりません**（4-499）。
    //
    // **増えたら赤**です。**返っていたものが返らなくなったら、それは
    // 失ったもの**です。
